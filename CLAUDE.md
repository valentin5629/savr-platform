...
- Sélection provider via `transporteurs.type_tms` — **valeurs réelles de l'enum (corrigé 2026-06-10, alignement §04/DDL)** : `mts1` (→ `adapter_mts1`), `a_toutes` (→ `adapter_everest`, **V1.1**), `autre` (→ `provider_manual` no-op, dispatch email/téléphone). **Spec de l'interface (méthodes, erreurs typées, idempotence) : `_DEV-FACING/01 - …/Interface logistique_provider V1.md`** — à lire avant de coder l'adapter.
