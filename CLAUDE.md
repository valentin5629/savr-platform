...
> **Accès depuis le repo (2026-06-11)** : les specs sont versionnées dans **`specs/`** à la racine du repo — `specs/cdc/` (= `_DEV-FACING`), `specs/tests/` (scénarios Gherkin), `specs/ddl-cible/schema_cible_v2.sql`, `specs/fixtures/`, `specs/manifests/` (couverture par module). Tout pointeur `_DEV-FACING/X` de ce fichier se lit `specs/cdc/X`. `specs/` est **dérivé** (sync depuis le Vault, commit `specs: sync`) — ne jamais l'éditer à la main. **Exception : `specs/manifests/` = artefacts *authored* (couverture par module), PAS issus du Vault → éditables, mais à maintenir en phase avec le CDC (grain livrable + `cdc_source_hash`, gates G2/G10/G11 — voir `30. Review Code/Backlog final priorisé/Lot 0 …`).** Conventions d'exécution complètes (test:module, check:coverage, mocks, sous-lots) : `specs/cdc/… 12 - Conventions exécution Claude Code` (copie de `09 - Roadmap exécution/12`).
...
1. **Data model V1 ⊂ data model archive** — **omissions OU divergences explicitement tracées dans `_Divergences/` avec plan de convergence V2** (jamais de structure divergente *non tracée* qui serait renommée/migrée en silence). Diff schéma bloquant. **Exception tracée (2026-06-24, arbitrage A6) : les 4 tables Bloc 7 (`integrations_logs`, `integrations_inbox`, `outbox_events`, `emails_envoyes`) divergent structurellement du DDL cible — convergence reportée V2, cf. `_Divergences/_traités/2026-06/BLOC7_20260624.md` *(chemin corrigé 2026-07-06 COH-17 — fichier archivé)*.** **✅ Prérequis levé (2026-06-08) :** DDL cible V2 exécutable écrit et gelé → `_DDL-CIBLE-V2/schema_cible_v2.sql` (**93 tables = 58 `plateforme.*` + 33 `tms.*` + 2 `shared.*`, 56 enums, 162 FK, 320 statements** ; validé pglast — **regelé 2026-06-10** : + `jobs_pdf`, + enums vérif SIRET/TVA ; **regelé bis 2026-06-11** : + `pesees_tournees`, outbox lease/claim ; **regelé 2026-07-06** : + `tms.outbox_events`, + `collecte_tournees.statut_execution`, `tms.pesees.idempotency_key` default retiré — comptes réconciliés COH-12, l'en-tête du `.sql` fait foi). Cf. `_DDL-CIBLE-V2/README.md` (périmètre = structure diffable sans RLS/triggers/vues ; **ambiguïtés A1-A7 toutes confirmées Val — 2026-06-09/10, plus aucun marqueur `/* AMBIGU */`**). Colonnes V1-only assumées (liste fermée, cf. Frontière G1) : `nb_camions_demande`, `code_transporteur_mts1`, `id_point_collecte_mts1`, table `pesees_tournees` (ajout 2026-06-11, revue adversariale INC-0). **DDL regelé bis 2026-06-11 (revue adversariale)** : outbox_events +`txid`/`claimed_until`/`requires_reconciliation`, + `pesees_tournees` — **compte courant après regel 2026-07-06 : 93 tables (58/33/2), 56 enums, 162 FK, 320 statements pglast OK** *(les ex-comptes « 89 tables / 311 statements » avaient dérivé, réconciliés COH-12)*. **Fichier DÉRIVÉ : à régénérer après toute modif d'un Data Model.** Le diff des migrations V1 se fait désormais contre ce fichier (nom ⊆ cible ? type identique ? pas de champ renommable ?). **Repriorisation conservée :** câbler d'abord garde-fou 3 (grep anti-couplage) + garde-fou 4 (test outbox par mutation), le diff schéma vient ensuite.
...
- **Émission côté TMS (V2) = outbox transactionnelle `tms.outbox_events`** (§08 §2bis + §04 TMS, arbitrage 2026-07-06 RC-M04-06) : toute mutation métier émettrice INSÈRE l'event dans sa transaction ; worker lease/claim §07 TMS §18bis (retry 3 paliers, head-of-line par agrégat, DLQ → M11) ; étendue aux jobs `everest.create`/`everest.cancel` M14 (COH-03 option A). **Le gel porte sur le wire (events/payloads/en-têtes) — le mécanisme d'émission est désormais spécifié** *(ajout 2026-07-06 COH-13)*.
...
> **Harnais qualité câblé et smoke-testé** (skill `cdc-dev-quality-loop` BUILD, 2026-06-08). Repo squelette = `savr-platform/` (monorepo pnpm+Turborepo prêt à pousser). Traçabilité + preuve smoke test : `_Harnais qualité dev/00 - Harnais câblé 2026-06-08.md`. Principe : consigne critique = mécanisme qui l'impose (test > hook > gate CI > reviewer), pas du texte espéré suivi. **Audit conformité code-vs-CDC (2026-06-23) → `30. Review Code/Backlog final priorisé/` : ~118 écarts (9 P0), cause racine = gates code-vs-manifeste jamais code-vs-CDC + manifestes au grain scénario. Remédiation = Lot 0 (gates `check:spec-deliverables`, `manifest-grain`, `schema-vs-cible`, colonne-DB, oracle sémantique, drift, `integration-contracts`, preuve-visuelle) en mode rapport puis durcis par cliquet. Backstop go-live = re-lancer `.claude/workflows/cdc-conformity-audit.mjs` (0 nouveau gap critique).**
...
- **Pré-merge (câblé 2026-09-22, hook `PreToolUse` `gate-merge.sh`)** : sur `gh pr merge`, rejoue `check-migration-timestamp.sh --merge` — contrôle **(C)**, le seul qui compare à `origin/main` **re-fetché à l'instant du merge**. (A) et (B) se jouent au commit et en CI, contre la cible telle qu'elle était alors ; entre la CI verte et le merge, `main` avance (PR #373, 2026-09-21 : 8 commits dont 3 migrations postérieures, pendant une seule revue — 6e occurrence de cette famille). Une migration mergée mal ordonnée fait échouer tout `supabase db push` suivant, et le seul remède du CLI (`--include-all`) applique AUSSI les migrations en attente des autres lots. **Seul un refus avéré (exit 2) bloque** ; tout autre code trace « contrôle non joué » sans bloquer (une branche ouverte avant ce lot porte un script qui ignore `--merge`). Le hook ne voit que l'outil Bash de Claude Code : ni l'UI GitHub, ni `gh pr merge --auto`, ni un terminal humain — le filet complémentaire est `migration-timestamp` en **status check requis** (`BRANCH_PROTECTION.md`, pas encore posé au 2026-09-22). Auto-tests non vacants : `check-migration-timestamp.sh --self-test` et `gate-merge.sh --self-test`, les deux en CI.
- **Merge `main`** : PR obligatoire + checks verts + 1 approbation, pas de bypass admin, pas de force push ; déploiement Vercel ; tag git.
- **Merge prod / déploiement prod = AUTORISÉ à Claude Code (assoupli 2026-09-03, décision Val)** — motif : **aucun utilisateur réel en prod aujourd'hui**, le coût d'un incident prod est nul. Claude Code peut merger sur `main`, laisser partir le déploiement prod et **appliquer les migrations Supabase prod** sans attendre Val, aux **conditions non négociables** : (1) CI verte + gate-pr OK (aucun bypass, aucun `--admin`, aucun force push) ; (2) migration prod **jamais destructive** (add column nullable OK ; `DROP`/`RENAME`/backfill lourd → STOP, demander Val) ; (2bis) **migration qui OUVRE ou ÉLARGIT un accès → STOP, demander Val** (ajout 2026-09-11, R1 revue sécurité #280) : `GRANT … TO anon|authenticated|PUBLIC`, policy créée ou modifiée pour être plus permissive (dont `USING (true)`), `DISABLE ROW LEVEL SECURITY`, fonction `SECURITY DEFINER` sans `REVOKE EXECUTE … FROM authenticated, anon, PUBLIC` (faille P0 #263), modification du hook JWT ou des triggers anti-escalade. Une migration qui touche ces objets **pour fermer** (REVOKE, policy plus restrictive, nouvelle fonction `SECURITY DEFINER` fermée) reste autorisée, avec GO `reviewer-rls-securite` sur le SHA **et** test pgTAP prouvant la fermeture. Claude Code ne pose **jamais** lui-même le marqueur CI `-- REVIEWED-DESTRUCTIVE:` ; (3) **jamais de seed en prod**, jamais de copie prod→dev non anonymisée ; (4) le déploiement prod est **annoncé à Val dans la réponse** (ce qui part, quelles migrations) ; (5) si les secrets prod (`savr-prod`) ne sont pas disponibles dans la session, la migration prod reste à Val — le merge/déploiement code, lui, passe quand même. ⚠ **Règle temporaire, à révoquer dès qu'un client réel est en production** → retour à « déploiement + migration prod = action manuelle Val (+ frère) ».
