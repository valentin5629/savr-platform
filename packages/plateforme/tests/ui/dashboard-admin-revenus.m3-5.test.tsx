...
describe('M3.5 / Dashboard Admin Bloc 2 Revenus (BL-P2-03)', () => {
  it(
    'M3.5/admin_revenus_7_colonnes — nom · type · nb/CA ZD · nb/CA AG · Total HT',
    async () => {
  ...
  it(
    'M3.5/admin_kpi_cartes_cliquables — chaque carte Bloc 1 lie vers /admin/collectes?chip= (miroir §11 §1.1)',
    async () => {
      render(<DashboardAdminPage />);
      await screen.findAllByText('Traiteur Alpha', undefined, ATTENTE_UI); // page hydratée (KPI chargés)
      const hrefs = screen
        .getAllByRole('link')
  ...
  it(
    'M3.5/admin_revenus_histogramme_monte — RevenusHistogramme n’est plus orphelin',
    async () => {
      render(<DashboardAdminPage />);
      expect(
        await screen.findByTestId('revenus-histogramme', undefined, ATTENTE_UI),
      ).toBeInTheDocument();
    },
    ATTENTE_CAS_MS,
  );
  ...
  it(
    'M3.5/admin_revenus_controls — titre bloc + champs de période présents, presets + export CSV retirés (revue E2E 2026-07-18)',
    async () => {
      render(<DashboardAdminPage />);
      await screen.findAllByText('Traiteur Alpha', undefined, ATTENTE_UI);
  ...
  it(
    'M3.5/admin_revenus_reinitialiser — « Réinitialiser » ramène au défaut 12 derniers mois',
    async () => {
      render(<DashboardAdminPage />);
      await screen.findAllByText('Traiteur Alpha', undefined, ATTENTE_UI);
      const { from, to } = defaultWindow();
  ...
  it(
    'M3.5/admin_revenus_tri — clic sur une colonne triable relance le fetch trié',
    async () => {
      render(<DashboardAdminPage />);
      await screen.findAllByText('Traiteur Alpha', undefined, ATTENTE_UI);
      fetchMock.mockClear();
