...
export async function seedMinimal(client: pg.Client): Promise<void> {
  ...
  // ── 13bis. Calcul CO₂ AG (UPDATE nu, sans transition de statut) ───────────
  // `trg_co2_ag_cloture` est AFTER UPDATE (…→cloturee) ; le seed insère les AG
  // déjà 'cloturee' → jamais déclenché. On NE PEUT PAS rejouer la transition : le
  // round-trip cloturee→realisee déclencherait `trg_pack_debit_realisee` (BEFORE
  // UPDATE OF statut) → double-débit du pack, casse seed:check. On peuple donc
  // co2_evite_kg + snapshot par un UPDATE qui NE NOMME PAS `statut` : les 3
  // triggers pack (BEFORE UPDATE OF statut) ne se déclenchent jamais. Formule +
  // snapshot = copie exacte de trg_co2_ag_cloture. Effet de bord voulu :
  // volume_repas_realise (absent en minimal) alimente AUSSI « Repas donnés » AG de
  // v_kpi_traiteur. En prod, la clôture EST un UPDATE de statut → le trigger tourne
  // normalement (aucune régression prod).
  await client.query(`
    -- volume_repas_realise sur les attributions AG clôturées (= volume estimé),
    -- seulement si absent (no-op quand déjà seedé, ex. seed_demo).
    ...
    [
      {
        ...
        payload: jsonb({ tourId: 'T1', status: 'OK' }),
        traite: true,
        traite_at: ts(-5, 6),
      },
      {
        ...
        payload: jsonb({ tourId: 'T2', status: 'IN_PROGRESSION' }),
        traite: false,
      },
    ],
    ['id'],
