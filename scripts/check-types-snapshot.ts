 ...
 *
 * Sens unique SUBI, pas choisi : le snapshot est généré depuis savr-dev, qui
 * porte en plus des objets créés à l'exécution — partitions annuelles fabriquées
 * par `f_ensure_partition_annee` (audit_log_2027…2031, integrations_logs_2027),
 * absentes des migrations par construction. Exiger l'égalité rendrait donc le
 * gate rouge en permanence.
 *
 * ⚠ ANGLE MORT ASSUMÉ, en contrepartie : un objet resté au snapshot mais SUPPRIMÉ
 * en base (migration `DROP`) continue de compiler. Une table fantôme n'est EN
 * PRATIQUE que du type mort (un DROP TABLE est bruyant, le code suit dans le même
 * lot), mais une COLONNE dropée est pire : `.update({ colonne_supprimee: … })`
 * compile, G7 compte 0, et PostgREST rejette au runtime — exactement la classe
 * d'échec que G7 existe pour attraper. Précédent réel : 11 colonnes retirées par
 * `20260629000000_plateforme_drop_legacy_packs_tarifs.sql`. Ce gate ferme le sens
 * MANQUANT (le plus large : une table entière invisible) ; le sens SUPPRIMÉ reste
 * couvert par la seule discipline « régénérer après toute migration ». Le fermer
 * mécaniquement demanderait de comparer le snapshot à une base réelle (celle-là
 * même que lit `supabase gen types`), donc un job avec DB. Ce n'est PAS le
 * périmètre de G6 (`check:schema-vs-cible`) : G6 compare les migrations au DDL
 * cible V2, jamais au snapshot — une colonne dropée le rend d'ailleurs plus vert,
 * pas plus rouge.
 *
 * Périmètre = tables + vues, PAS les fonctions ni les enums :
 *   - fonctions : `supabase gen types` n'exporte pas les fonctions trigger
 *     (105 noms de fonction créés par les migrations sur les 3 schémas générés,
 *     contre 69 exportés dans le snapshot) — un contrôle d'inclusion produirait
 *     une trentaine de faux positifs structurels. Le cas qui compte (RPC appelée
 *     par l'app et absente du snapshot) est déjà rouge chez G7, et ne peut plus
 *     être mal diagnostiqué maintenant que la fraîcheur du snapshot est
 *     cliquetée.
