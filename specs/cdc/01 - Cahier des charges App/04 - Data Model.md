...
- **Colonnes exposées** : `collecte_id`, `volume_repas_realise`, `association_nom`, `association_ville` — **rien d'autre**. Sont explicitement **exclues** toutes les colonnes opérationnelles internes de `attributions_antgaspi` : `confirmation_transporteur`, ranking / scoring de l'algo d'attribution (§06.09), références MTS-1 / Everest, horodatages de dispatch.
...
- **Implémentation** : vue **SECURITY DEFINER** (owner privilégié, pas de `security_invoker`) + GRANT SELECT au rôle `gestionnaire_lieux`. En `SECURITY INVOKER` la vue hériterait de `aa_select` et rendrait zéro ligne : l'option n'existe pas. Modèle repris de `v_traiteurs_gestionnaire` (#363) et `v_referentiel_traiteurs`.
...
- **Points d'appel à repointer sur la vue** — 6, mesurés le 2026-09-21 (`grep -rl attributions_antgaspi packages/plateforme/src/app/api/v1/gestionnaire/`), à câbler en **un seul lot** : `evenements/[id]` (détail événement, sous-bloc AG), `evenements/` (colonne « Repas donnés », §06.05 §2), `dashboard/` (KPI « repas donnés »), `pack-ag/` (écran Mon pack AG), `traiteurs/` (liste Traiteurs), `traiteurs/[id]` (fiche traiteur).
- **pgTAP** : `v_attributions_gest_own_lieu_ok` (le gestionnaire voit l'attribution d'une collecte AG sur SON lieu, événement programmé par un traiteur tiers) / `v_attributions_gest_autre_lieu_denied` (lieu hors `organisations_lieux` → aucune ligne) / `v_attributions_gest_colonnes_whitelist_ok` (aucune colonne de scoring ni de confirmation transporteur exposée) / `T18` inchangé.
- **Statut 2026-09-21** : **spécifiée, NON implémentée**. Tant que la vue n'est pas sur `main`, le §06.05 §3 reste faux sur 6 écrans.
