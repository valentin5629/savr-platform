...
- **Implémentation** : vue **SECURITY DEFINER** (owner privilégié, pas de `security_invoker`) + GRANT SELECT à `authenticated` (tout rôle client). Le SELECT direct sur `plateforme.organisations` reste inchangé (aucun élargissement).
...
**Partitionnement** *(migration `20260710000000_plateforme_purge_logs_retention_partitions.sql`)* : `PARTITION BY RANGE (created_at)`, partitions annuelles ; 2026 → 2031 pré-provisionnées one-shot, les suivantes par `f_ensure_partition_annee` (qui pose aussi la garde de vidage). **Pas de partition `DEFAULT`** : un INSERT hors plage échoue. Rétention légale 5 ans (§4 de `07 - Observabilité/06`).
**Écriture** : **jamais par l'API directement** — INSERT via triggers DB et code serveur (`SERVICE_ROLE` / fonctions `SECURITY DEFINER`). Aucun UPDATE, DELETE ni vidage de table (append-only, immuable) — garantie portée par les triggers `trg_audit_log_immuable` et `trg_audit_log_vidage_interdit` + REVOKE, opposable aux rôles applicatifs.
