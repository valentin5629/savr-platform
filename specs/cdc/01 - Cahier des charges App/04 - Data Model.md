...
**Partitionnement** *(structure : `20260611171641_plateforme_bloc7_integrations.sql` ; pré-provisionnement et helper : `20260710000000_plateforme_purge_logs_retention_partitions.sql`)* : `PARTITION BY RANGE (created_at)`, partitions annuelles ; 2026 → 2031 pré-provisionnées one-shot, les suivantes par `f_ensure_partition_annee` — ⚠ qui **ne pose aucune garde de vidage** au 2026-09-22 (son corps ne crée que la partition et y active/force la RLS) : l'extension reste à faire. **Pas de partition `DEFAULT`** : un INSERT hors plage échoue. Rétention légale 5 ans (§4 de `07 - Observabilité/06`).
**Écriture** : **jamais par l'API directement** — INSERT via triggers DB et code serveur (`SERVICE_ROLE` / fonctions `SECURITY DEFINER`). Aucun UPDATE, DELETE ni vidage de table (append-only, immuable) — UPDATE/DELETE portés par `trg_audit_log_immuable` + REVOKE (migration `20260921200000`) ; la garde de vidage de table `trg_audit_log_vidage_interdit` est portée par `20260922110000_plateforme_audit_log_garde_vidage.sql`, **PR #383 encore ouverte au 2026-09-22 — non acquise sur `main`**. Opposable aux rôles applicatifs.
