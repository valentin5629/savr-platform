...
|----------------------------|------|---------------|--------|---------|
| `acces_details` | text | Admin Savr | Ops + Admin TMS (via column-level GRANT) | Instructions accès : badge, code, interphone, contact gardien, **digicode/bip parking** (ex-`code_acces`), **notes stationnement texte libre** (ex-`parking`). Champ "carnet d'accès terrain" partagé Plateforme commercial + TMS opérationnel. **⚠ Précision 2026-09-16 (arbitrage Val)** : l'ownership « Admin Savr » porte sur la **valeur de RÉFÉRENCE dans `lieux`**. Il n'empêche **pas** une surcharge **per-collecte** par le programmeur via `collectes.lieu_overrides` (§06.01 §2.a, 9 champs éditables) : le `lieux` officiel n'est jamais modifié, la surcharge est tracée et bornée. `acces_details` ne fait **pas** partie de `R_lieux_admin_only_fields`. |
...
| `raison_sociale` | text | NULL | **Ajout 2026-06-11 (audit data model — colonne fantôme régularisée)** — Raison sociale de l'organisation, référencée par `v_registre_dechets` (`traiteur_raison_sociale`, F4 2026-06-07) et `v_referentiel_traiteurs` (F5 2026-06-07) qui la lisaient sans qu'elle existe. ⚠ Depuis le 2026-09-22, `v_referentiel_traiteurs` ne projette plus `raison_sociale` (revue sécurité PR #363) — elle ne sert plus qu'au fallback du libellé unique `nom` ; seul `v_registre_dechets` la lit encore en propre. Saisie à l'onboarding (ou par l'agence pour une fiche shadow — la décision shadow 2026-05-07 mentionnait déjà « nom + raison sociale »). NULL toléré (fallback affichage = `nom` via `COALESCE(raison_sociale, nom)` dans les 2 vues). ⚠ Ne remplace pas `entites_facturation.raison_sociale` (entité juridique de facturation, source de vérité facture) — celle-ci porte la raison sociale par entité, celle-là la raison sociale « par défaut » de la fiche organisation (registre, référentiel). |
...
| `logo_url` | text | | Clé R2 `<bucket>/logos/<uuid>.(png\|jpg)` (bucket applicatif de l'environnement, préfixe `logos/` ; format imposé par trigger, cf. doctrine `*_url`). Couvre traiteurs, agences, gestionnaires de lieux et clients finaux. |
...
| `photos_urls` | text[] | | URLs des photos du lieu (stockées dans Cloudflare R2) |
...
| `chauffeur_nom` | text | nullable | Nom du chauffeur. **V1** : capté auto par l'adapter MTS-1 (`transporters[].firstname`/`lastname`) ou saisi Admin. |
| `chauffeur_telephone` | text | nullable | **Renseigné manuellement (Admin) en V1** *(divergence PLAQUES 2026-07-15)* — MTS-1 n'expose pas le téléphone chauffeur (`transporters[]` sans phone ni email, cf. [[Adapter MTS-1 (MyTroopers) — relevé as-built Bubble]] §6). Capté auto si un provider futur l'expose (seam `transporters[].phone`). |
| `accompagnant_nom` | text | nullable | **(ajout 2026-07-15, divergence PLAQUES — infos accès chauffeur V1)** Nom de l'accompagnant éventuel. **Saisie Admin uniquement** (MTS-1 = 1 transporteur par dispatch, pas de 2e contact auto). |
| `accompagnant_telephone` | text | nullable | **(ajout 2026-07-15, divergence PLAQUES)** Téléphone de l'accompagnant. Saisie Admin uniquement. |
...
| `numero_rup` | text | NULL | **Ajout 2026-09-14 (arbitrage Val, divergence M2.4 `numero-rup-source`)** — N° RUP si l'association est Reconnue d'Utilité Publique. **Source de l'instantané `attestations_don.association_numero_rup`**, qui était défini sans colonne source (le batch AG lisait `associations.numero_rup` inexistante → toute la sélection échouait et aucune attestation n'était générée). Saisi dans la **modale association** du back-office Admin (§06.06 §5). Facultatif : l'attestation Cerfa 2041-GE n'affiche la mention RUP que s'il est renseigné. Reprise Bubble à prévoir si le champ y existe. |
...
| `logo_url` | text | NULL | **Ajout R17b 2026-07-02** — Clé R2 `<bucket>/logos/<uuid>.(png\|jpg)` du logo association (format imposé par trigger), affiché dans les rapports AG. |
...
| `type_tms` | enum | NOT NULL | **Ajout 2026-05-08** — Enum `mts1` / `a_toutes` / `autre` / `par_mail` / `par_telephone` (**ajout R17b 2026-07-02** — `par_mail`/`par_telephone` = transporteurs hors TMS routés `provider_manual`, validation manuelle Admin). Détermine le mode de dispatch + le bouton affiché en Bloc 0 Attribution Prestataire §06 §3 (Envoyer à MTS-1 / Envoyer à A Toutes! / Manuel email+téléphone). **IMMUABLE après création (2026-09-16, arbitrage Val)** : un transporteur garde le même type de TMS jusqu'au TMS Savr natif ; le changer rouvrirait la fuite inter-provider et rendrait silencieuses les E2/E3 des collectes en cours. Trigger `trg_transporteur_cols_immuables`. Fusionne les ex-champs `process_creation_collecte`, `process_creation_collecte_detail`, `type_tms` jamais physiquement créés en DB Bubble. |
| `prestataire_logistique_id` | uuid | FK → **`shared.prestataires`**, NULL, UNIQUE | **Ajout 2026-09-16 — V1 only (décision Val 2026-06-25 option B, R5 ; documenté par la divergence M1.1b_20260916)** — pont entre le référentiel transporteur Plateforme et l'identité prestataire côté provider. **C'est la marque de provider que lisent les adapters** pour ne traiter que leurs propres tournées (cf. [[Interface logistique_provider V1]] Cloisonnement par provider) : un transporteur `mts1` ou `a_toutes` sans ce lien est **inopérant** — 100 % de ses événements partent en file d'erreur. **Requis si `type_tms ∈ {mts1, a_toutes}`.** UNIQUE : un prestataire n'admet qu'un transporteur. **IMMUABLE après création** (trigger `trg_transporteur_cols_immuables`, au même titre que `type_tms`) — pour en changer, créer un nouveau transporteur. Déprécié V2 (le TMS natif résout ce lien). |
| `code_transporteur_mts1` | text | NULL | **Ajout 2026-05-29 — V1 only (propagation [[08 - APIs et intégrations#3bis. API Plateforme ↔ MTS-1]])** — `carrierShareableCode` côté MTS-1, identifie ce transporteur lors du **dispatch de la tournée** `POST /v3/dispatch/{tourId}/toCarrier` (payload `{ carrierShareableCode }` — route corrigée 2026-09-04 sur la doc OpenAPI V3 ; flux réconcilié relevé as-built 2026-06-06). Récupérable via `GET /v3/carrier`. **Requis si `type_tms = 'mts1'`** (cf. [[05 - Règles métier#R_code_mts1_requis]]). Déprécié en V2 (gardé en lecture pour audit). |
...
| `logo_client_organisateur_url` | text | | Clé R2 `<bucket>/logos/<uuid>.(png\|jpg)` (format imposé par trigger). Uploadé par le traiteur_commercial ou traiteur_manager au moment de la programmation. Utilisé dans le rapport de recyclage PDF. Priorité : si `client_organisateur_organisation_id` est renseigné ET que `organisations.logo_url` existe, le logo de l'organisation prime. Sinon, ce champ est utilisé. |
...
| `aucun_repas_photo_url` | text | NULL | URL Cloudflare R2 de la photo du lieu prise par le chauffeur comme preuve de présence si `statut = realisee_sans_collecte` |
...
| `dirty_tms` | boolean | NOT NULL, défaut `false` | **Ajout 2026-05-07 — « émission S7 » renommée « émission dispatch » 2026-06-07 (F5 session test-scenarios §06.06 : S7 = webhook TMS→App `plaque-saisie`, le sens sortant App→TMS est le dispatch E1 / réémission §08 §10.1)** — `true` si la collecte a subi une modification métier (date, heure, lieu, flux, contrôle d'accès, info supplémentaire) **après** dernière émission dispatch et **avant** renvoi explicite. Reset à `false` à la prochaine émission dispatch. **Les champs ÉVÉNEMENT propagés au TMS (contacts, pax) NE passent PAS par `dirty_tms`** : ils émettent E2 `collecte.modifiee` immédiatement, par collecte dispatchée, via `fn_modifier_evenement` (modèle immédiat, cf. [[05 - Règles métier]] Précision M1.2 du 2026-06-26). **⚠ Cette liste de champs est NORMATIVE pour le payload sortant E2** *(2026-09-16)* : y ajouter un champ oblige à lui donner une contrepartie dans le payload du provider, ou à inscrire son exclusion **et sa raison** dans [[08 - APIs et intégrations]] §3bis.8. Un test-cliquet lit la liste du trigger et rougit sinon. **❓ Question ouverte (non tranchée)** : la prose ci-dessus énumère six items (« date, heure, lieu, **flux**, contrôle d'accès, info supplémentaire ») pour **cinq** comparaisons dans `fn_set_collectes_dirty_tms`, qui ne teste **aucune colonne de flux**. Soit « flux » désigne une donnée réellement propagée au TMS (les `stuffs` ZD ?) et le trigger est incomplet — le payload E2 avec lui ; soit c'est un reliquat de rédaction à supprimer. Délibérément **pas supprimé en silence**. Sert au KPI dashboard Admin "Collectes modifiées sans renvoi TMS" (§06 §1 Bloc 1). Géré par trigger DB `set_collectes_dirty_tms` sur UPDATE des champs COLLECTE propagés au TMS (le trigger porte sur `collectes`, il ne voit jamais `evenements`). |
...
| `lieu_overrides` | jsonb | NULL | **Ajout 2026-05-25 (audit sobriété §04 — C1, option b actée §05 L1027 + addendum 2026-05-03 §3)** — Valeurs de lieu corrigées par le programmeur au formulaire (override per-collecte, clé/valeur). Lu par le TMS via E1 pour figer `tms.collectes_tms.lieu_snapshot`. NULL si aucune modification. Tranche la question ouverte de l'addendum 2026-05-03 §3 (option b retenue). **Forme contrainte (2026-09-16)** : `chk_collectes_lieu_overrides_textuel` + `collectes_lieu_overrides_valide_chk` (migration `20260915160000`) — allowlist des 9 clés éditables, valeurs **textuelles uniquement** (chaîne, tableau de chaînes pour `flux_autorises`, ou `null`), longueurs bornées, aucun caractère de contrôle (`[[:cntrl:]]` fait foi). Une valeur invalide déjà stockée n'est **pas** une surcharge : le lieu officiel est transmis à sa place et le champ redevient propageable par E5. Cf. [[06 - Fonctionnalités détaillées/01 - Formulaire de programmation de collecte]] §2.a. |
...
**RLS (audit RLS V1 2026-06-05 — **révisée 2026-09-14, arbitrage Val**)** — vaut pour `grilles_tarifaires_zd`, `tarifs_zero_dechet` et `tarifs_packs_ag` : lecture **staff** (`admin_savr`, `ops_savr`, prédicat `plateforme.f_is_staff()`), écriture **`admin_savr` only**. La justification d'origine « pas de donnée par orga » **était fausse** : depuis la refonte 2026-05-26, une base négociée se matérialise par une **grille dédiée du catalogue affectée à une organisation** (rattachement `organisations.grille_tarifaire_zd_id`), donc le catalogue porte de la donnée commerciale par client — le nom de grille compris. Aucun consommateur applicatif n'est impacté (100 % `service_role`) et le prix n'est toujours pas affiché au formulaire (règle UI Sujet 5). Le détail négocié vit dans `tarifs_negocie` + `factures_collectes.tarif_detail`. Cf. [[09 - Authentification et permissions#A5 — `grilles_tarifaires_zd` + `tarifs_zero_dechet` + `tarifs_packs_ag`]].
...
>
> **Régénération manuelle traiteur_manager (tranché 2026-06-07, F3 lot ⑫ ; canal aligné 9.1.16 le 2026-07-07)** : passe par une **Next.js API Route SERVICE_ROLE** (contrôle applicatif du périmètre, mêmes 4 chemins que la policy A8 SELECT) — aucune écriture client directe, policy `rr_write_admin` inchangée. Test P1 bloquant : régénération cross-org → 403.
...
> 
> **Spec PDF synthèse** (contenu) : conservée et déplacée dans [[12 - Reporting et exports]] §Synthèses à la demande. La génération est synchrone (Next.js API Route + Railway/Puppeteer), le PDF n'est plus persisté en DB — l'utilisateur le télécharge et le conserve lui-même.
...
| `pdf_url` | text | NOT NULL | URL Cloudflare R2 |
...
- **Colonnes exposées** *(restreint 2026-09-22 — revue sécurité PR #363, migration `20260922080000_plateforme_referentiel_traiteurs_libelle_unique.sql`)* : `id`, `nom` — **et rien d'autre**. `nom` est un **libellé unique** = `COALESCE(NULLIF(organisations.nom, ''), organisations.raison_sociale)` (nom commercial, ou raison sociale à défaut si aucun nom commercial n'est saisi). La colonne `raison_sociale` a été **retirée de la projection** : la vue (`security_invoker = false`, `GRANT SELECT` à `authenticated`) l'exposait pour **tous** les traiteurs actifs non-shadow à **tout** utilisateur connecté, au-delà du nom seul autorisé par §06.05 (« ne voit pas les données commerciales/personnelles des traiteurs au-delà du nom/logo ») et §06.11 §3 (« Traiteur opérationnel : {{nom}} », un seul libellé). Ni `siret`, ni `logo_url`, ni colonne interne. ⚠ Un futur écran qui aurait besoin de la raison sociale légale d'un traiteur tiers (hors registre) devra passer par une **vue/route dédiée avec sa propre revue sécurité** — ne pas rouvrir celle-ci.
- **Périmètre** : `type = 'traiteur' AND est_shadow = false AND actif = true` — les trois conditions font partie de la garantie : un traiteur désactivé ne doit plus apparaître dans le référentiel servi à tous les rôles clients. Les fiches shadow de l'agence restent lues en direct via la policy `organisations` dédiée (créatrice seule).
- **Implémentation** : vue en `security_invoker = false` (owner privilégié — **jamais** une fonction `SECURITY DEFINER`, cf. faille P0 #263) **+ `WITH (security_barrier = true)`**, puis `REVOKE ALL … FROM PUBLIC, anon, authenticated` **avant** `GRANT SELECT … TO authenticated` (tout rôle client). Libellé unique réel : `COALESCE(NULLIF(btrim(o.nom), ''), o.raison_sociale)`. Le SELECT direct sur `plateforme.organisations` reste inchangé (aucun élargissement).
- **Non concerné par cette restriction** : le **Registre réglementaire** (§06.03, `v_registre_dechets`) lit `raison_sociale` **directement** sur `organisations` (join `top.raison_sociale`), pas via cette vue — il continue de porter la raison sociale du traiteur opérationnel comme producteur juridique du déchet (F4 2026-06-07), inchangé.
...
### Vue SQL : `v_attributions_gestionnaire` *(ajout 2026-09-21 — divergence M3.2, option (b) tranchée Val)*

Vue whitelist servant la lecture des données AG (Anti-Gaspi) par le rôle `gestionnaire_lieux` sur les collectes qui ont eu lieu **dans ses lieux**, y compris quand l'événement a été programmé par un **traiteur tiers**. Nécessaire car la policy `aa_select` de `plateforme.attributions_antgaspi` (§09, décision **C-1**) n'a **pas de branche « lieu »** : sans cette vue, le sous-bloc AG du détail événement (§06.05 §3) et 5 autres écrans affichent un zéro silencieux dès que le gestionnaire n'est pas lui-même le programmateur — c'est-à-dire dans le cas nominal de ce persona.

- **Colonnes exposées** : `collecte_id`, `volume_repas_realise`, `association_id`, `association_nom`, `association_ville` — **rien d'autre**. `association_id` est nécessaire au calcul de distance à la volée du §06.05 §3 (lecture de `associations.latitude/longitude`, déjà ouverte à tout `authenticated` par `asso_read`). Sont explicitement **exclues**, en les nommant par leur vrai nom de colonne : `transporteur_id`, `branche_attribution`, `confirmation_transporteur`, `mode_validation`, `valide_par`, `valide_at`, `poids_repas_kg`, `motif_override`, `motif_override_libre`.
- **Périmètre** : attributions dont la `collecte_id` porte sur un événement dont le `lieu_id` appartient au gestionnaire (`lieu_id IN (SELECT lieu_id FROM organisations_lieux WHERE organisation_id = auth.jwt()->>'organisation_id')`). Bornage **explicite dans la vue** — il ne peut pas venir de la RLS, que la vue contourne par construction.
- **Implémentation** : vue en `security_invoker = false` (défaut PostgreSQL — **jamais** une fonction `SECURITY DEFINER`, cf. faille P0 #263) **+ `WITH (security_barrier = true)`** (points d'appel PostgREST avec filtres client : sans barrière, un filtre « leaky » de l'appelant s'évalue avant le WHERE de la vue). Le WHERE porte **deux** bornages cumulatifs, pas un : (1) `plateforme.f_app_role() = 'gestionnaire_lieux'` — garde de rôle **obligatoire**, car `organisations_lieux` ne contraint pas le type d'organisation (aucun CHECK, et la route admin n'impose rien) : sans elle, un `client_organisateur` rattaché à un lieu lirait les attributions AG et contournerait C-1 ; (2) `collecte_id` dont l'événement porte un `lieu_id ∈ organisations_lieux` de l'organisation. Privilèges : `REVOKE ALL ON plateforme.v_attributions_gestionnaire FROM PUBLIC, anon, authenticated;` **puis** `GRANT SELECT … TO authenticated` (il n'existe pas de rôle Postgres `gestionnaire_lieux` : c'est un rôle métier porté par le claim `user_role`). Modèle à copier **en entier** : `v_traiteurs_gestionnaire` (#363, `20260921090000_plateforme_organisations_traiteurs_gestionnaire_vue.sql`) — surtout **pas** la formulation de `v_referentiel_traiteurs`, qui est un référentiel public sans scoping.
- **`aa_select` n'est JAMAIS élargie** : la décision **C-1** du §09 reste vraie au niveau de la table, le pgTAP bloquant `T18 attributions_ag_gestionnaire_denied` (`rls_0_4_smoke.test.sql:283`) reste vert. Élargir la policy ouvrirait la table entière et relève du cas « migration qui ÉLARGIT un accès → STOP » (CLAUDE.md §12, R1 revue sécurité #280).
- **Points d'appel à repointer sur la vue** — **8**, mesure corrigée le 2026-09-22 (le grep du 2026-09-21 était borné au dossier `gestionnaire/`), à câbler en **un seul lot** : `evenements/[id]` (détail événement, sous-bloc AG), `evenements/` (colonne « Repas donnés », §06.05 §2), `dashboard/` (KPI « repas donnés »), `pack-ag/` (écran Mon pack AG), `traiteurs/` (liste Traiteurs), `traiteurs/[id]` (fiche traiteur), **plus les deux routes partagées** `GET /api/v1/dashboards/evolution` (`loadEvolution`) et `GET /api/v1/dashboards/blocs` (`loadBlocs`), qui lisent `attributions_antgaspi` via `packages/plateforme/src/lib/dashboards/loaders.ts` et servent **aussi** `traiteur_manager`, `traiteur_commercial` et `agence`, pour qui `aa_select` répond correctement. **Arbitrage Val 2026-09-22 — option A : branchement par rôle dans `loaders.ts`.** Les deux routes partagées lisent `v_attributions_gestionnaire` quand `ctx.role === 'gestionnaire_lieux'`, et `attributions_antgaspi` en direct pour `traiteur_manager` / `traiteur_commercial` / `agence` (comportement actuel, correct pour eux puisque `aa_select` leur répond). La garde de rôle `f_app_role() = 'gestionnaire_lieux'` de la vue est **conservée** — c'est elle qui impose ce branchement, puisqu'elle rend la vue vide pour ces 3 rôles. Les deux chemins n'ayant pas la même forme — `loadBlocs` lit un embed **imbriqué** `attributions_antgaspi(volume_repas_realise, association_id, associations!association_id(id, nom, ville))` là où la vue expose `association_nom` / `association_ville` **à plat** — ils sont normalisés vers une forme commune côté `loaders.ts`, dans le lot de câblage. *(Annule et remplace l'arbitrage du 2026-09-21 « les deux chemins sont interchangeables ; pas de branchement par rôle dans `loaders.ts` ».)*
- **pgTAP** : `v_attributions_gest_own_lieu_ok` (le gestionnaire voit l'attribution d'une collecte AG sur SON lieu, événement programmé par un traiteur tiers) / `v_attributions_gest_autre_lieu_denied` (lieu hors `organisations_lieux` → aucune ligne) / `v_attributions_gest_colonnes_whitelist_ok` (aucune colonne exclue exposée : `transporteur_id`, `branche_attribution`, `confirmation_transporteur`, `mode_validation`, `valide_par`, `valide_at`, `poids_repas_kg`, `motif_override`, `motif_override_libre`) / `v_attributions_gest_role_non_gestionnaire_denied` (**garde de rôle D1** : une organisation `client_organisateur` **rattachée au lieu** via `organisations_lieux` ne rend aucune ligne, avec contrôle de non-vacuité — le même rattachement doit rendre ≥ 1 ligne au rôle `gestionnaire_lieux`) / `T18` inchangé.
- **Statut 2026-09-21** : **spécifiée, NON implémentée**. Tant que la vue n'est pas sur `main`, le §06.05 §3 reste faux sur **8** écrans (mesure corrigée 2026-09-22).
...
| `nb_organisations_distinctes` | integer | Nombre d'organisations programmatrices du segment. **Garde** (et non plus simple audit) : un segment n'est publié que si ce compteur ET celui des traiteurs opérationnels valent ≥ 3 — cf. « RLS / k-anonymat ». |
...
| `p_traiteur_ids` | uuid[] | Filtre sur `evenements.traiteur_operationnel_organisation_id` (le traiteur qui OPÈRE l'événement — pas `organisation_id`, qui est l'organisation programmatrice et peut être une agence, un gestionnaire de lieux ou un client organisateur) *(corrigé 2026-09-22 — la spec citait `organisation_id`, l'as-built filtre le traiteur opérationnel depuis `20260705130000`)* |

**RLS / k-anonymat (durci 2026-09-22 — arbitrage Val, revue sécurité)** : la fonction applique **deux** seuils cumulatifs côté serveur dans son `HAVING` final :
1. `nb_collectes_segment >= 5` (inchangé) ;
2. **`>= 3` acteurs distincts**, mesurés comme le **minimum** entre `COUNT(DISTINCT evenements.organisation_id)` (organisations programmatrices) et `COUNT(DISTINCT evenements.traiteur_operationnel_organisation_id)` (traiteurs opérationnels). Seuil aligné sur `f_taux_recyclage_moyen_parc` (`p_nb_acteurs_min = 3`) et sur le k-anonymat ≥ 3 acteurs du §11.

Le seuil n° 1 seul ne suffisait pas : 5 collectes d'un **même** acteur le franchissent, et la moyenne publiée décrit alors cet acteur, pas le parc — un concurrent pouvait l'isoler en resserrant les filtres (mesuré sur `seed_demo` : 10 segments à organisation unique et 20 à traiteur opérationnel unique dès qu'on filtre sur un lieu). Ni l'un ni l'autre n'est contournable côté front. Plus le demandeur restreint les filtres benchmark, plus le risque de masquage augmente — c'est le compromis assumé de l'option D. Migration `20260922210000_plateforme_benchmark_k_anonymat_organisations.sql`.

> **Limites résiduelles mesurées, NON fermées** (hors périmètre, à arbitrer) : **L1** fuite par différence (4 requêtes + connaissance externe suffisent encore à reconstituer le ratio d'un concurrent) ; **L2** domination interne au segment (un acteur à ~97 % du tonnage est publié à 1,3 % de son ratio privé — un seuil de comptage ne voit pas les poids ; il faudrait un critère de concentration) ; **L3** garde compétitive `p_traiteur_ids` absente en base pour le rôle `agence` (tenue côté application seulement).
...
- K-anonymat appliqué sur `benchmark_kg_pax` uniquement — ≥ 5 collectes **et ≥ 3 acteurs distincts**, hérité de `f_benchmark_kg_pax_zd` (le `ratio_user` est par définition une donnée du demandeur, pas anonymisable).

**Garde** (fail fast, migration `20260923090000` — PR #395) : la fonction est `SECURITY DEFINER`, **la RLS de l'appelant ne s'y applique donc PAS** et le contrôle d'accès est explicite dans le corps : (1) `f_app_role()` NULL → `RAISE EXCEPTION 'Role applicatif absent (acces refuse)'` ; (2) hors `admin_savr`/`ops_savr`, organisation applicative NULL → refus, puis `v_org IS DISTINCT FROM v_evt_org AND v_org IS DISTINCT FROM v_evt_top` → `RAISE EXCEPTION 'Collecte not accessible'`. ⚠ Écrire cette fonction « avec la RLS » revient à l'écrire **sans aucun contrôle d'accès** ; les tests NULL sont indispensables (`NULL NOT IN (…)` vaut NULL, donc n'entre dans aucun `IF`).

> **Note as-built (mise à jour 2026-09-22, PR #387)** : la fonction est implémentée, recâblée sur la nouvelle signature de `f_benchmark_kg_pax_zd` (valeur servie = moyenne pondérée) et **consommée par la fiche collecte §06.04** (`GET /api/v1/traiteur/collectes/{id}/benchmark`). Les noms de colonnes de sortie ci-dessus **sont** ceux de l'as-built depuis `20260922100000_plateforme_benchmark_single_collecte_colonnes_canoniques.sql` (`taille_evenement`, `ratio_user`, `benchmark_kg_pax`, `nb_collectes_segment`) — les anciens noms `bracket`/`valeur_kg_pax`/`median_kg_pax` n'existent plus. ⚠ Revenir à ces anciens noms sans revenir aussi sur le code applicatif est un mode de panne **silencieux** (clé absente de la réponse JSON, aucune erreur levée).

#### Fonctions d'alimentation des filtres benchmark — `f_benchmark_lieux_parc` / `f_benchmark_traiteurs_parc` *(ajout 2026-09-23 — divergence SECU-RLS sync v3 §(E) ; absentes de tout le CDC jusqu'ici, migration de référence `20260923090000_plateforme_benchmark_gardes_role_fail_closed.sql`, PR #395)*

Les deux listes de valeurs du filtre benchmark dédié (lieux du parc, traiteurs du parc) sont servies par deux fonctions `SECURITY DEFINER` distinctes de `f_benchmark_kg_pax_zd`. Elles agrègent cross-organisations, donc elles ne peuvent pas s'appuyer sur la RLS de l'appelant : **leur seul contrôle d'accès est une liste blanche de rôles écrite en tête de fonction, fail-closed**.

| | `f_benchmark_lieux_parc` | `f_benchmark_traiteurs_parc` |
|---|---|---|
| Sortie | liste des lieux du parc (id, nom) utilisables comme valeurs de `p_lieu_ids[]` | liste des traiteurs du parc (id, nom) utilisables comme valeurs de `p_traiteur_ids[]` |
...
⚠ `NULL NOT IN (…)` vaut NULL en SQL : le test « rôle absent » doit être une garde **distincte et première**, sinon un jeton sans `user_role` traverse la liste blanche sans lever.
...
| `consumer` | text | | Identifiant du consommateur. **Deux familles cohabitent sur la même table (2026-09-11, Option A tranchée Val — divergence M2.3)** : (1) **famille logistique** — `adapter_mts1` / `adapter_everest` / `provider_manual` en V1, `adapter_tms_natif` en V2 ; (2) **famille jobs applicatifs** — `attribution_job` (emails d'attribution AG n°16 et n°18). Valeurs terminales techniques : `noop_no_remote`, `admin_skip`. |
...
Un worker (Vercel Cron / Next.js API Route, `SERVICE_ROLE`) claim les lignes éligibles, POST vers MTS-1, puis renseigne `consumed_at`/`consumer`. **Test garde-fou 4** : `savr-platform/supabase/tests/outbox_par_mutation.test.sql` (1 event par mutation + atomicité rollback).

**Isolation par famille de consumer (ajout 2026-09-11, Option A tranchée Val — divergence M2.3 `outbox-deux-consommateurs`)**
La validation d'une attribution AG insère, dans la même transaction, **deux** events sur le même agrégat (la collecte) : `attribution.validee` (consumer `attribution_job`) puis `collecte.creee` (famille logistique). Sans isolation, le worker logistique réclamait `attribution.validee`, ne le reconnaissait pas, le marquait `done` avec `consumer = 'noop_no_remote'` → **aucun email d'attribution ne partait jamais, en silence**. Règles figées :

- **`fn_claim_outbox_batch` (worker logistique) exclut `consumer = 'attribution_job'`** et calcule son head-of-line **au sein de la seule famille logistique**.
- **`fn_claim_outbox_attribution_batch`** : claim dédié aux jobs applicatifs, même pattern lease/claim + garde de visibilité `txid < xmin`, mêmes paliers de retry (5 min / 1 h / 24 h) puis `dead`.
- **Le head-of-line est calculé au sein d'une même famille, jamais entre familles** : un email d'attribution en échec **ne doit jamais bloquer le dispatch logistique** de la collecte (sinon un email raté empêche l'envoi du camion).
- Colonne de statut = **`statut`** (pas `status`) et valeur DLQ = **`dead`** (pas `dlq`) — l'enum `outbox_statut_enum` fait foi.
- **Alerte DLQ** : un event `dead` déclenche l'alerte Slack **critique** quelle que soit sa famille, `attribution_job` inclus (cf. [[07 - Observabilité/03 - Alertes]], précisé 2026-09-14).

*Alternatives écartées : (B) file dédiée type `jobs_pdf` pour les jobs applicatifs (écart DDL cible, tracé G1) ; (C) corriger seulement le cron (course entre crons 5 min / 15 min + blocage du dispatch par le head-of-line).*
...
- **Alerte anticipée collecte imminente (ajout 2026-06-10, challenge logistique+onboarding)** : si l'event concerne une collecte dont `date_collecte < now() + interval '24h'` **ET** `attempts ≥ 2` → **alerte Slack `#savr-alerts-critique` immédiate, sans attendre la DLQ**. Motif : avec les paliers 5 min / 1 h / 24 h, une panne MTS-1 de 2 h à J-0 laisse l'échec silencieux entre le palier 1 h et le palier 24 h alors que la collecte est le soir même (camion absent). Cf. [[07 - Observabilité/03 - Alertes]].
  **⚠ Extension aux events `lieu` (2026-09-16, arbitrage Val — divergence OBS_20260915_alerte-anticipee-events-lieu)** : le prédicat ci-dessus était écrit autour d'une collecte (il lit `collectes.date_collecte`), et le code en tirait un garde `if (aggregate_type !== 'collecte') return`. **E5 `lieu.champ_critique_modifie` porte `aggregate_type = 'lieu'`** : c'était le seul event outbox que l'alerte anticipée ne voyait jamais — ~25 h de silence (3 paliers complets) avant la seule alerte DLQ. Désormais, pour un event `aggregate_type='lieu'`, la **date de référence** est la **plus proche `date_collecte` parmi les collectes futures non terminales du lieu** (jointure `evenements.lieu_id`, déjà écrite dans `AdapterMts1.updateLieu`). Même seuil (`attempts ≥ 2` ET date < now()+24h), même canal. Cas couvert : l'Admin corrige une adresse à 17 h, une collecte s'y tient le soir même, l'E5 échoue en boucle — le camion partirait à l'ancienne adresse sans que personne soit prévenu à temps.
...
**RLS** : écriture `SERVICE_ROLE` uniquement (Next.js API Route `send-email` + webhook `/webhooks/resend/events`) ; `SELECT` `admin_savr` ; deny tout autre rôle (PII). Cf. [[09 - Authentification et permissions]] (bloc ajouté 2026-06-07).
...
| Colonne | Type | Contraintes | Notes |
|---------|------|-------------|-------|
| `id` | bigint | Séquence (`audit_log_id_seq`) ; **PK composite `(id, created_at)`** | *(corrigé 2026-09-21 — l'énoncé « uuid, PK simple » ne correspondait plus à la base depuis le partitionnement `20260710000000` : `PARTITION BY RANGE` impose que la clé de partitionnement figure dans la PK)* |
...
**Partitionnement** *(structure : `20260611171641_plateforme_bloc7_integrations.sql` ; pré-provisionnement et helper : `20260710000000_plateforme_purge_logs_retention_partitions.sql`)* : `PARTITION BY RANGE (created_at)`, partitions annuelles ; 2026 → 2031 pré-provisionnées one-shot, les suivantes par `f_ensure_partition_annee`, **qui pose aussi `trg_audit_log_vidage_interdit` sur chaque partition créée** (migration `20260922200000_plateforme_audit_log_garde_vidage.sql`, PR #383 mergée le 2026-09-22 — le trigger d'instruction n'étant pas cloné par PostgreSQL, sa pose explicite est la seule garantie). **Pas de partition `DEFAULT`** : un INSERT hors plage échoue. Rétention légale 5 ans (§4 de `07 - Observabilité/06`).
**Écriture** : **jamais par l'API directement** — INSERT via triggers DB et code serveur (`SERVICE_ROLE` / fonctions `SECURITY DEFINER`). Aucun UPDATE, DELETE ni vidage de table (append-only, immuable) — UPDATE/DELETE portés par `trg_audit_log_immuable` + REVOKE (migration `20260921200000`) ; la garde de vidage de table `trg_audit_log_vidage_interdit` est portée par `20260922200000_plateforme_audit_log_garde_vidage.sql` (PR #383, mergée le 2026-09-22) — posée sur le parent, sur les partitions existantes et par `f_ensure_partition_annee` sur les partitions à venir. Opposable aux rôles applicatifs.
**RLS** : `SELECT` `admin_savr` + `ops_savr` ; deny tout autre rôle ; INSERT/UPDATE/DELETE/TRUNCATE deny pour tous les rôles applicatifs (cohérent matrice §09 « Audit log : écriture — (trigger DB) »).
