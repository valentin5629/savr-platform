...
La base de prix ZD vient du **catalogue** `grilles_tarifaires_zd` (voir section 04). Plusieurs grilles coexistent (méthodes `paliers` | `fixe_variable`), chaque organisation est rattachée à une grille via `organisations.grille_tarifaire_zd_id` (NULL → grille `est_defaut`). Chaque ligne de grille porte une **formule affine** `montant_fixe_ht + montant_par_pax_ht × pax` sur sa tranche.

**Grille par défaut « Standard paliers »** (`est_defaut=true`, seed initial) :

| Tranche pax | montant_fixe_ht | montant_par_pax_ht | Prix HT                            |
| ----------- | --------------- | ------------------ | ---------------------------------- |
| 1 – 250     | 450 €           | 0                  | 450 €                              |
| 251 – 500   | 600 €           | 0                  | 600 €                              |
| 501 – 750   | 800 €           | 0                  | 800 €                              |
...
Le palier > 1 000 (1 €/pax) est désormais exprimé proprement en affine (`montant_fixe_ht=0`, `montant_par_pax_ht=1`, borne inférieure 1 001) — plus de cas spécial dans le code.

**Exemple grille « Forfait + variable »** : une ligne `[1, null]` avec `montant_fixe_ht=200`, `montant_par_pax_ht=1` → 200 € + 1 €/pax. Affectée à une organisation via `grille_tarifaire_zd_id`.
...
**Modification tarifaire** : pour modifier une grille, l'Admin Savr ferme la grille (`grilles_tarifaires_zd.valide_jusqu_au`) et en crée une nouvelle (entête + lignes). Pas de modification rétroactive ; les collectes passées conservent leur calcul via `tarif_detail`.
...
- **Sous-branche selon délai** : délai `< regle_ag_seuil_h2_minutes` (défaut 90 min) → A Toutes! vélo express (Everest 75) `ag_velo_express` ; sinon → A Toutes! vélo programmé (Everest 71) `ag_velo_programme`.
...
**Stockage de la branche** : `attributions_antgaspi.branche_attribution` (text NOT NULL, 8 valeurs canoniques + `province` pour hors IDF).
...
**Déclencheurs** _(2e déclencheur ajouté 2026-06-07 — test scenarios §06.01 F2, arbitrage Val)_ :
...
### Annulation d'une collecte AG : recrédit automatique _(refonte 2026-05-08)_
...
| `realisee` → `cloturee`    | **Automatique** — clôture immédiate. Bordereaux, attestations et rapport RSE générés au batch J+1 à 6h (voir section 6)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Système                                                                             |
...
#### R_statut_collecte_multi_tournees _(refonte multi-camions 2026-05-25)_
...
- **Tous les tours `CANCELED`/`KO`** → logique refus existante au niveau collecte : `statut_tms = 'rejetee_par_prestataire'` + notif Admin + retour file (mapping [[08 - APIs et intégrations]] §3bis.6 inchangé).
...
**Statuts autorisés** : `collectes.statut IN ('programmee', 'validee')` ET `collectes.statut_tms IN ('non_envoye', 'a_attribuer', 'attribuee_en_attente_acceptation', 'acceptee')`. Verrouillage UI dès `collectes.statut IN ('en_cours', 'realisee', 'realisee_sans_collecte', 'cloturee', 'annulee')` _(`manquee` retiré — audit sobriété §04 2026-05-25 D1)_.
...
**Règles** _(modélisation révisée audit sobriété §04 2026-05-25 D1 — statut `manquee` supprimé)_ :
...
- Si annulation reçue **< 12h avant l'heure de collecte** OU **après mandat prestataire** : facturation **plein tarif** — ZD : facture standard ; AG : **débit d'un crédit pack** via trigger `trg_pack_debit_annulation_tardive` _(confirmé Val 2026-06-07 — F2, cette section fait foi ; mécanique détaillée §3 « Débit d'un crédit » déclencheur 2 ; sans pack actif → alerte Admin + arbitrage manuel, F3)_
...
**Règles** _(révision audit sobriété §04 2026-05-25 D1 — statut `en_reexamen` supprimé)_ :
...
4. Après validation admin : `factures.statut = en_attente_pennylane`, push API Pennylane ; `emise` + `pennylane_id` renseigné **uniquement après succès du push** _(corrigé test-scenarios 2026-06-07 — résidu stale qui décrivait `emise` avant le push ; le flux unique [[06 - Fonctionnalités détaillées/08 - Génération et édition facture (Admin)]] §2 fait foi)_

### Zéro-Déchet — Mode mensuel groupé _(réécrit décision F2 test-scenarios 2026-06-07, arbitrage Val — §06.08 §3 fait foi, ex-génération manuelle Admin)_
...
### Alerte pesées anormales (Admin Savr)

**Objectif** : détecter les erreurs de saisie TMS avant la génération des documents, pendant la fenêtre de 24h d'embargo.

**Déclencheur** : à réception du webhook `collecte-terminee` du TMS avec `statut_final = realisee` (immédiate, avant le batch J+1). **S'applique uniquement aux collectes ZD** (5 flux V1 : biodechet, emballage, carton, verre, dechet_residuel) — il y a toujours des déchets à peser, donc une valeur hors plage indique une erreur de saisie. Les collectes AG ne sont pas concernées : le cas "Aucun repas à collecter" est déjà géré par un statut dédié (`realisee_sans_collecte`) et une alerte Ops dédiée côté TMS (voir [[02 - Cahier des charges TMS/03 - Périmètre fonctionnel TMS#M11]]).

**Logique** : calcul du ratio `(Σ poids_net_kg par flux × 1000) / nb_pax` = g/pax pour chaque flux. Le calcul porte sur le **total agrégé du flux sur la collecte** (somme de toutes les pesées individuelles du même flux — un chauffeur peut faire N pesées successives pour un même flux si le contenant ne rentre pas en 1 fois sur la balance). Comparaison avec la plage [min, max] dans `parametres_algo`. Alerte si valeur < min (données manquantes / erreur saisie) ou > max (valeur aberrante).
...
Min = 10% du max. Modifiables par Admin Savr sans redéploiement. Toute modification loggée dans `audit_log`.

**Ce que l'alerte ne fait pas** : elle ne bloque pas la génération des documents. Elle donne à l'Admin Savr la fenêtre de 24h pour corriger avant le batch automatique.

### R_taux_recyclage — Calcul du Taux de recyclage _(ajout 2026-05-06)_
...
### R_co2_calcul — Calcul de l'impact carbone CO₂ (induit / évité / net / énergie) _(ajout 2026-06-04, Sujet 3)_
...
### R_co2_emballage_mix — FE emballage dérivé du mix _(ajout 2026-06-04, Sujet 3)_
...
### R_co2_snapshot_fige — Reproductibilité (snapshot figé à la clôture) _(ajout 2026-06-04, Sujet 3)_
...
### R_co2_ag — Calcul de l'impact carbone CO₂ AG (repas détournés) _(ajout 2026-06-04 bis)_
...
### R_marge_zd_traiteur — Calcul de la Marge générée ZD (KPI dashboard traiteur) _(ajout 2026-05-07)_
...
- `Σ factures HT = 0` mais `Σ pax > 0` (collectes cloturées sans facture émise — cas attente facturation) → marge = revenu pur, badge info "{{n}} collectes en attente de facturation" sous la carte. _(Précision 2026-06-07 — test scenarios §06.04 F3, arbitrage Val)_ : le badge s'affiche dès que **n ≥ 1**, où `n` = nombre de collectes ZD `cloturee` du périmètre filtré sans facture `emise`/`payee` rattachée — **y compris en facturation partielle** (le cas Σ HT = 0 est couvert par cette règle générale).
...
### R_dechets_labo_estimes — Estimation des déchets produits au labo du traiteur _(ajout 2026-05-22)_
...
### R_volume_estime_ag_calcule — Calcul automatique du volume estimé de repas AG _(ajout 2026-05-07)_
...
### R_flux_par_defaut_zd — Aucune saisie flux ZD côté traiteur, peuplement post-pesée _(ajout 2026-05-07, consolidation)_
...
### R_revenus_imputation_organisation — Imputation revenus tableau Dashboard Admin _(ajout 2026-05-07)_
...
### R_compatibilite_vehicule_lieu — Compatibilité véhicule transporteur ↔ lieu _(ajout 2026-05-08)_
...
>
> Tout autre profil (traiteur*\*, agence*_, gestionnaire\__, client*organisateur*\*) **ne doit jamais voir ces colonnes** :
>
...
- Factures achat pack AG : `FPK-YYYY-NNNNN` _(ajouté 2026-06-07 test-scenarios — série existante §06.08 §7 + §04, absente de cette liste par dette doc)_
...
### R_code_mts1_requis — Code transporteur MTS-1 obligatoire si `type_tms = 'mts1'` _(ajout 2026-05-29, propagation §3bis)_

**Objectif** : garantir qu'un transporteur dispatché via MTS-1 dispose bien de son `carrierShareableCode`, sans quoi le dispatch de la tournée `POST /v3/tours/{tourId}/dispatch` (payload `{ carrierShareableCode }`) échoue _(flux réconcilié sur le relevé as-built — propagation §08 §3bis 2026-06-06)_.
...
| **Collecte programmée par un tiers** _(2026-05-07)_                                                                                                                                                   | `traiteur_operationnel_organisation_id` (manager + commerciaux du traiteur opérationnel)                                                                                                                                                     | **Info-only**, pas de validation requise. Template `collecte_programmee_tiers` _(ajouté 2026-06-07, F2 session test-scenarios §06.02)_ + récap (date, lieu, flux, contact, programmateur). Déclenché uniquement si `evenements.organisation_id ≠ traiteur_operationnel_organisation_id` ET si le traiteur opérationnel n'est pas une fiche shadow. Si shadow → pas de destinataire (silencieux). |
| **Collecte modifiée ou annulée par un tiers** _(2026-05-07)_                                                                                                                                          | `traiteur_operationnel_organisation_id` (manager + commerciaux)                                                                                                                                                                              | Idem ci-dessus. Template `collecte_modifiee_tiers` _(ajouté 2026-06-07, F2 — variable `type_changement` modification/annulation)_. Déclenché uniquement si action effectuée par programmateur ≠ traiteur opérationnel.                                                                                                                                                                           |
| Collecte modifiée                                                                                                                                                                                     | `collectes.created_by`                                                                                                                                                                                                                       | Récap diff avant/après                                                                                                                                                                                                                                                                                                                                                                           |
| Collecte annulée                                                                                                                                                                                      | `collectes.created_by` (template `collecte_annulee`) + Admin Savr (template `admin_collecte_annulee`, _ajouté 2026-06-07 F2_) + (si tiers) `traiteur_operationnel_organisation_id` (`collecte_modifiee_tiers`, `type_changement=annulation`) | Confirmation annulation.                                                                                                                                                                                                                                                                                                                                                                         |
| Rapport post-collecte disponible                                                                                                                                                                      | `collectes.created_by`                                                                                                                                                                                                                       | Lien espace client + résumé impact                                                                                                                                                                                                                                                                                                                                                               |
| Inscription                                                                                                                                                                                           | User                                                                                                                                                                                                                                         | Template `verification_email` — lien d'activation valide 24h _(F5 corrigée 2026-06-07 : l'ancienne ligne « Première connexion → bienvenue + lien vérif » conflatait 2 templates)_                                                                                                                                                                                                                |
...
| Publication rapport post-collecte | Batch J+1 à 6h (embargo H+24 strict)                                               | Rapport non accessible avant H+24. Alerte pesées anormales déclenchée dès réception webhook TMS pour permettre correction avant génération. |
...
| KPI                                                                                                     | Granularité                               |
| ------------------------------------------------------------------------------------------------------- | ----------------------------------------- |
| Taux de recyclage _(ZD uniquement, formule à captation par filière, cf. R_taux_recyclage ci-dessous)_   | Par lieu / traiteur / événement / période |
...
- **ZD** : grille du catalogue affectée à l'organisation programmatrice (`organisations.grille_tarifaire_zd_id`, NULL → grille `est_defaut`). Base = `montant_fixe_ht + montant_par_pax_ht × pax` sur la ligne couvrant `evenements.pax` **(pax_collecte retiré V1 le 2026-05-29 — pax unique niveau événement)**.
...
Issu de la refonte du formulaire de programmation §06.01. Les règles ci-dessous gouvernent les nouveaux comportements introduits. _(Note : `R_type_evenement_libre` retirée V1 le 2026-05-26 — propagation Sujet 4, cf. ci-dessous.)_
...
### R_lieu_modif_pending _(simplifié 2026-05-25 — audit sobriété §04 B1)_
...
### R_date_evenement_auto_derive _(ajout 2026-05-29)_
...
### R_pax_collecte _(ajout 2026-05-29 — Retiré V1 2026-05-29)_
...
### R_collecte_evenement_rattachement _(ajout 2026-05-21, D1)_
...
**Audit** : chaque cascade est loggée dans `audit_log` _(nom canonique — résidu `audit_logs` corrigé 2026-06-07 F1, table définie [[04 - Data Model#Table : `audit_log`]])_ (action `controle_acces_cascade_upgrade`, user déclencheur = traiteur, lieu cible).
...
- **Tarification ZD** : catalogue de grilles (`grilles_tarifaires_zd`, méthodes `paliers` | `fixe_variable`, formule affine `montant_fixe_ht + montant_par_pax_ht × pax`), grille affectée par organisation (`grille_tarifaire_zd_id`, NULL = grille défaut « Standard paliers » : 1-250→450 / 251-500→600 / 501-750→800 / 751-1000→1000 / >1000→1€/pax). Composition base × remises figée à la facturation via `tarif_detail` + `montant_ligne_ht` (refonte 2026-05-26).
...
- **Notifications V1** : email transactionnel uniquement, destinataire = programmeur de la collecte, événements déclencheurs métier + système. _(Template « plaque chauffeur » T+3h retiré V1 — la notification client de plaque a été supprimée ; la plaque reste saisie côté TMS pour traçabilité interne uniquement, cf. matrice §9.)_
...
- **Alerte pesées anormales** : plage min/max en g/pax par flux. Seed V1 : biodéchets 15-150, carton 2-20, déchet résiduel 40-400, verre 20-200, emballage 20-200 (min = 10% du max). Alerte email Admin si hors plage. Configurable sans redéploiement via `parametres_algo`.
