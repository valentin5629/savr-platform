...
| **Horaires compatibles** : l'heure de collecte tombe dans un créneau [début, fin[ d'un jour coché « Ouvert » de `associations.horaires_ouverture` (créneau fin ≤ début = passe minuit, la partie après minuit compte pour le lendemain ; 00:00→00:00 = 24h/24 ; créneau incomplet ou mal formé = ignoré ; horaires non renseignés = **pas d'exclusion**) — arbitrage Val 2026-09-21 | Critère éliminatoire |
...
| `programmee` → `rejetee_par_prestataire` | Refus du transporteur avant acceptation : refus Everest synchrone ou webhook (`mission_failed` / annulation externe), ou tous les tours MTS-1 `CANCELED`/`KO` (§08 §3 / §3bis.6). `statut_tms = rejetee_par_prestataire` + alerte Admin in-app « réattribution requise » | Adapter logistique (automatique) |
| `rejetee_par_prestataire` → `programmee` | **Réattribution Ops** (dispatch Admin, `fn_dispatcher_collecte`) — `statut_tms` repasse à `non_envoye` ; la collecte reprend le parcours normal (arbitrage Val 2026-09-17) | Admin / Ops Savr |
...
- **Tous les tours `CANCELED`/`KO`** → **si la collecte est en cours d'exécution (`programmee`, `validee`, `en_cours`)** : `collectes.statut = 'rejetee_par_prestataire'` (visibilité dashboard, décision Val 2026-06-15) + `statut_tms = 'rejetee_par_prestataire'` + **alerte Admin in-app « réattribution requise »**. Une collecte `annulee`, `annulation_demandee` (la demande est conservée, l'Admin la tranche — arbitrage Val 2026-09-17) ou terminale n'est pas modifiée : ni `statut`, ni `statut_tms`, ni alerte. Retour file **Ops-driven** (réattribution/reprogrammation manuelle), pas de remise auto en file — la file `/pending` exige `statut = 'programmee'` (mapping [[08 - APIs et intégrations]] §3bis.6).
...
4. **Prix final = base × (1 − max(remise_pct))** — **pas de cumul** : parmi toutes les remises éligibles (scope organisation et scope gestionnaire, une ou plusieurs chacune), seule la plus élevée s'applique (arbitrage Val 2026-09-17). La base est ferme (le client paie sa grille).
...
**Exemple** : Butard (grille « Forfait + variable » 200 € + 1 €/pax) programme 300 pax chez Viparis (remise gestionnaire −5 %) → base = 200 + 300 = 500 € → final = 500 × 0,95 = **475 € HT**. Si Butard a en plus une remise propre de −15 %, seule celle-ci s'applique : 500 × 0,85 = **425 € HT** (pas de cumul).
