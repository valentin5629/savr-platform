...
Table `plateforme.audit_log`, **append-only et immuable** (aucun UPDATE, DELETE ni vidage de table). Garantie portée par `trg_audit_log_immuable` + REVOKE pour UPDATE/DELETE (migration `20260921200000`) ; la garde de vidage de table `trg_audit_log_vidage_interdit` est portée par `20260922200000_plateforme_audit_log_garde_vidage.sql` (PR #383, mergée le 2026-09-22) ; opposable aux **rôles applicatifs** (`postgres` hors garantie). Le trigger de vidage est un trigger d'instruction : PostgreSQL ne le clone pas sur les partitions, il est donc posé explicitement sur le parent, sur chaque partition existante, **et par `f_ensure_partition_annee`** sur les partitions annuelles à venir (la même migration a étendu son corps, qui ne créait auparavant que la partition en y activant/forçant la RLS). Écriture **jamais par l'API directe** : INSERT via triggers DB et code serveur (`SERVICE_ROLE` / `SECURITY DEFINER`). RLS : `SELECT` = `admin_savr` + `ops_savr` ; deny tout autre rôle.
