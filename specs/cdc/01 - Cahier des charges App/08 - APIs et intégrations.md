...
**Statut** : Draft V1 — mise à jour architecturale 2026-04-23 (atelier tech avec frère) — **propagation revue sobriété §08 TMS COMPLÈTE Blocs A+B+C+D 2026-05-01** — **revue sobriété §08 App 2026-05-31 (A1 webhooks MTS-1 informatifs non implémentés V1 / A2 extract MTS-1 différé V2 / B1 Pennylane polling-only / B2 règle acceptation dédupliquée source §3 / C1 bug retry 5→3 / C2 retry source unique §6 / D1 enum `integrations_logs.system` +everest+mts1 / D2 enum `integrations_inbox.source` +mts1) + réponses éditeur MTS-1 intégrées (Q3bis-1 header webhook / Q3bis-2 refus=UPDATE / Q3bis-4 DEMO)**
**Dernière mise à jour** : 2026-05-08 (refonte enums Plateforme — `lieux.stationnement` enum 4 valeurs "type d'emplacement" → enum 3 valeurs "difficulté d'accès" `facile/difficile/tres_difficile`, `lieux.acces_office` text libre → enum 3 valeurs idem, `lieux.type_vehicule_max` + `transporteurs.types_vehicules` + `tournees.type_vehicule` alignés sur enum unifié `velo_cargo/camionnette/fourgon/vul/poids_lourd`. Ajout colonnes `lieux` admin/ops only (`commentaire_lieu`, `siren`, `email_gestionnaire`, `reference_citeo`) **strictement exclues** du payload S7 `lieu_snapshot`. Ajout `transporteurs.type_tms` enum `mts1/a_toutes/autre`. Cf. memory `project_refonte_back_office_admin_2026_05_08`.)
...
| **Collecte terminée** (réalisée OU aucun repas) | `POST /webhooks/tms/collecte-terminee` | `collecte_id, tournee_id, type (cloture \| correction — ajout audit cohérence 2026-07-06, aligné payload S5 [[../02 - Cahier des charges TMS/08 - Contrat API Plateforme-TMS|§08 TMS]] déclencheur (c) étendu), statut_final (realisee \| realisee_sans_collecte), pesees[] (uniquement flux réellement pesés — flag `presume_non_pese` retiré revue sobriété 2026-04-29 avec suppression `flux_prevus`), photos_collecte[], rolls, signature_asso, aucun_repas` |
| Incident collecte | `POST /webhooks/tms/incident` | `incident_id, collecte_id, tournee_id, type_incident (5 valeurs dont client_annule_avant_arrivee — motif unique avant arrivée, cf. [[../02 - Cahier des charges TMS/08 - Contrat API Plateforme-TMS|§08 TMS S9]]), gravite (warning \| critical), description, photos[], appels_effectues[], imputable_a, statut_collecte_apres (realisee \| realisee_sans_collecte \| echec_acces \| inchange \| annulee), geofence_status (avant_arrivee \| sur_place \| apres_cloture)` — *payload miroité complet + effet Plateforme précisés audit cohérence 2026-07-06 (arbitrage Val)*. **Effet Plateforme** : alerte Ops Savr + commercial en charge du compte + trace `audit_log`. Si `statut_collecte_apres ∈ (echec_acces, annulee)` → `collectes.statut = 'annulee'` + `motif_incident` (= description S9) + `incident_imputable_a` (mappé depuis `imputable_a`) + annulation flux downstream (bordereau, attestation) ; facturation selon règles annulation [[05 - Règles métier]] §4bis (imputable client = plein tarif ZD / débit pack AG ; imputable prestataire = pas de facturation). Cas type : `client_annule_avant_arrivee` + `geofence_status=avant_arrivee` (collecte TMS `planifiee → annulee`, R6.1 TMS). Si `inchange` : statut collecte non modifié. |
...
**Webhook `collecte-terminee` unifié** : un seul webhook gère les deux cas métier. Discriminé par `type` puis `statut_final` *(complété audit cohérence 2026-07-06 — le champ `type` existait §08 TMS depuis 2026-04-22 mais n'était pas miroité ici)* :
- `type = cloture` (défaut) — clôture terminale :
...
- **`type = correction` (V2 — toute source, étendu 2026-07-06 arbitrage Val RC-M05-04)** : re-push post-clôture émis par le TMS pour un ajustement Ops Savr TMS **OU** une pesée tardive insérée après la dérivation `realisee` (item offline DLQ rejoué — trigger `trg_pesee_tardive_s5_correction` §04 TMS). **Effet Plateforme** : mise à jour des pesées existantes + **régénération automatique** du bordereau/attestation si déjà générés (`version` incrémentée, ancien PDF archivé) + recalcul `taux_recyclage`/rapport RSE + **alerte Ops Savr « Correction pesée reçue depuis TMS »**. La régénération n'est donc plus exclusivement Admin-manuelle (cf. [[05 - Règles métier]] §6 + [[12 - Reporting et exports]]).
...
- `409 Conflict` — **deux codes distincts** (cf. codes normalisés §08 TMS), même traitement Plateforme : **alerte Ops, pas de retry**, la modification locale Plateforme reste appliquée (pas de rollback), réconciliation manuelle Ops (téléphone + M04 W2/W7). **Aucune notification automatique au traiteur** (doctrine §05 §4 « Ops fait le filtre », arbitrage Val 2026-07-06) :
  - `collecte_non_modifiable` : la collecte est passée à un statut non modifiable côté TMS entre la décision Plateforme et la réception du PATCH (ex : passée à `en_cours`).
  - **`collecte_sur_tournee_active` (ajout 2026-07-06, arbitrage Val RC-M04-07 option a)** : collecte encore `planifiee` mais rattachée via `collecte_tournees` à au moins une tournée `acceptee`/`en_cours` — en multi-camions, le 409 s'applique dès qu'UNE tournée liée est active.
...
Plateforme (validation attribution AG IDF par Admin Savr ou auto-accept)
  ...
  → calcul branche §06.09 §2.3 → transporteur résultant
    ...
    → si transporteur.type_tms = 'mts1' → appel MTS-1 (cf. §3bis ci-dessous)
      ...
      → rejet → `statut_tms = 'rejetee_par_prestataire'` + `collectes.statut = 'rejetee_par_prestataire'` (visibilité dashboard, décision Val 2026-06-15) + alerte Admin in-app « réattribution requise ». Retour file **Ops-driven** (réattribution/reprogrammation manuelle depuis l'alerte), pas de remise auto — la file `/pending` exige `statut = 'programmee'`
```
...
- Le rejet (HTTP error synchrone, webhook async Everest, ou refus Marathon) positionne `statut_tms = 'rejetee_par_prestataire'`, pose `collectes.statut = 'rejetee_par_prestataire'` (visibilité dashboard, décision Val 2026-06-15), déclenche une **alerte Admin in-app « réattribution requise »**. Le retour en file est **Ops-driven** (réattribution/reprogrammation manuelle depuis l'alerte), pas une remise automatique.
...
**Spec technique V1 (implémentée M2.5)** : services actifs Everest = **71** (vélo programmé), **74** (vélo express — ex-75 abandonné), **77** (camion express — branche attribution à confirmer, DIV-3 pending Val), **91** (camion fallback Marathon). Auth clé API + secret en Supabase Vault. Retry policy 3 paliers 5 min / 1h / 24h. Webhook = signal → re-fetch API (pas de HMAC, secret dans URL). Échec d'écriture de l'état métier → alerte Ops in-app `everest_webhook_non_enregistre`, réponse 500, inbox non marquée traitée ; un event déjà présent en inbox mais **non** traité est retraité au rejeu (doublon = ligne inbox `traite=true` uniquement) — arbitrage Val 2026-09-21. [À confirmer auprès d'Everest : relance sur réponse non-2xx ?] Gate levée 2026-06-15.
...
- `mission_failed` / annulation externe **avant acceptation** (`statut_tms = attribuee_en_attente_acceptation`) → `statut_tms = rejetee_par_prestataire` + `collectes.statut = rejetee_par_prestataire` (visibilité dashboard) + **alerte Admin in-app « réattribution requise »**.
- **Mission courante (arbitrage Val 2026-09-17)** : un webhook ne modifie la collecte (acceptation, rejet, `realisee_sans_collecte`) que si sa mission est la mission courante — collecte attribuée à un transporteur `type_tms = 'a_toutes'` **ET** tournée de la mission liée à la collecte **ET** `tournees.external_ref_commande = mission_id`. Sinon : `everest_missions` mise à jour, collecte intacte, trace `integrations_logs` ; alerte Ops in-app `everest_mission_hors_attribution` si l'event annonce un vélo actif (`mission_dispatched`, `mission_pickedup`) — risque de double passage, la réattribution V1 n'annulant pas la course A Toutes!. Tournée liée mais sans `external_ref_commande` (référence pas encore committée par l'adapter) : ni courante ni ancienne → alerte in-app `everest_webhook_non_enregistre`, réponse 500, inbox non traitée, retraitement au rejeu. Chaque écriture `collectes` porte sa garde dans le WHERE (0 ligne = no-op).
  ...
  ── 1. Créer la commande ──────────────────────────────────────────────
    → POST /v3/customerOrders
        payload : orderDate (date de collecte), timezone, serviceTime,
                  ...
                  orderNumber = collecte.reference + '-' + rang [+ '-r' + n si le rang a été refusé puis réattribué, n = numéro de tentative ≥ 2 porté par `tournees.reference_interne`] (clé fonctionnelle de corrélation, UNIQUE par commande ; rang=1 si mono-camion ; arbitrage Val 2026-09-17 : la réattribution réinitialise en place la tournée refusée),
...
| Refus / annulation transporteur — au polling : `customerOrderStatus ∈ {CANCELED, KO}` (ou, en V1.1/V2 webhook, `tour/update` eventType `CANCELED`/`UNVALIDATED` au niveau tournée) | `CANCELED` / `KO` | `rejetee_par_prestataire` | `rejetee_par_prestataire` (visibilité dashboard, décision Val 2026-06-15) — alerte Admin « réattribution requise », retour file Ops-driven |
...
- **Refus transporteur** : détecté **au polling V1** par lecture du `customerOrderStatus` qui passe à `CANCELED`/`KO` (la tournée dispatchée n'est pas acceptée — `tour.status.dispatch` rejeté / `appointmentStatus = RESPONSE_KO`). → `statut_tms = 'rejetee_par_prestataire'` + `collectes.statut = 'rejetee_par_prestataire'` (visibilité dashboard) + **alerte Admin in-app « réattribution requise »** (retour file Ops-driven, motif preset). Uniquement pour une collecte `programmee`/`validee`/`en_cours` : un ordre CANCELED/KO remonté sur une collecte `annulee` ou `annulation_demandee` n'écrit rien (arbitrage Val 2026-09-17). En V1.1/V2 (webhooks), le refus remonte au niveau **tournée** via `tour/update` (`CANCELED`/`UNVALIDATED`), le dispatch étant désormais porté par la tournée et non par une délégation de commande. Q3bis-2 **fermée**.
...
- Dédup : event déjà appliqué (même `svix-id`, ligne inbox `traite=true`) → no-op idempotent. Une ligne inbox `traite=false` est retraitée au rejeu.
- Échec de lecture/écriture en base (lecture `emails_envoyes`, MAJ statut, trace de l'anomalie `resend_id` inconnu, marquage inbox) → **500**, inbox non marquée traitée (Resend relance). **Pas d'alerte V1** — le rejeu svix suffit (arbitrage Val 2026-09-21).
