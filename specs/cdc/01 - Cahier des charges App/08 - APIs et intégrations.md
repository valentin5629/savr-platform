...
| | | **Reporté V1.1 (revue sobriété §08 App 2026-05-31 B1)** — V1 = polling J+1 uniquement (un seul chemin de code, latence ≤ 24h acceptable pour le recouvrement). Le webhook `invoice.paid` sera évalué en V1.1 si le besoin de temps réel se confirme. |
...
- **Refus transporteur** : détecté **au polling V1** par lecture du `customerOrderStatus` qui passe à `CANCELED`/`KO` (la tournée dispatchée n'est pas acceptée — `tour.status.dispatch` rejeté / `appointmentStatus = RESPONSE_KO`). → `statut_tms = 'rejetee_par_prestataire'` + `collectes.statut = 'rejetee_par_prestataire'` (visibilité dashboard) + **alerte Admin in-app « réattribution requise »** (retour file Ops-driven, motif preset). En V1.1/V2 (webhooks), le refus remonte au niveau **tournée** via `tour/update` (`CANCELED`/`UNVALIDATED`), le dispatch étant désormais porté par la tournée et non par une délégation de commande. Q3bis-2 **fermée**.
