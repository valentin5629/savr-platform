...
**Event-driven via webhooks** uniquement. **Pas de polling fallback V1** _(supprimé revue sobriété §08 Bloc A 2026-05-01 A4 — retry policy 3 paliers (5 min / 1h / 24h, simplifié Bloc B B1 ex-5 paliers) + dédup `integrations_inbox` 7j (Bloc B B5) couvrent les pannes <24h, intervention manuelle au-delà)_.

**Données accessibles en lecture directe cross-schema** (pas de webhook V1) :

- Coût tournée TMS via vue `plateforme.v_courses_logistiques` _(remplace ex-webhook S6, revue sobriété 2026-05-01 A2)_
- Stocks rolls traiteurs TMS via vue `plateforme.v_stocks_rolls` _(remplace ex-webhook S8, revue sobriété 2026-05-01 A3)_
...
**Authentification** : Mutual HMAC-SHA256 + JWT signé clé secrète partagée. **Rotation annuelle manuelle V1** (retournement atelier 2026-04-23 vs semestrielle). Détails auth (`Authorization`, `X-Savr-Signature`, `X-Savr-Timestamp`, `X-API-Version`) dans [[02 - Cahier des charges TMS/08 - Contrat API Plateforme-TMS#Authentification et sécurité]]. _(Header `Idempotency-Key` retiré revue sobriété Bloc C 2026-05-01 C4 — duplication avec `body.event_id`, dédup serveur lit le payload directement.)_

### Flux TMS → Plateforme (events entrants)

| Événement TMS                                                        | Endpoint Plateforme                    | Payload clé                                                                                                                                                                                                                                                              |
| -------------------------------------------------------------------- | -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Collecte démarrée                                                    | `POST /webhooks/tms/collecte-en-cours` | `collecte_id, tournee_id, demarree_le, chauffeur_id` _(champ `geoloc` retiré revue sobriété Bloc B 2026-05-01 B4 — la Plateforme n'utilise pas la géoloc, retard traité côté TMS M11. Bonus RGPD minimisation)_                                                          |
...
**Réponse webhook** : 2xx = ack. Si erreur, TMS retry selon la retry policy uniforme **3 paliers : 5 min / 1h / 24h** _(simplifié revue sobriété Bloc B 2026-05-01 B1 — ex-5 paliers)_.
...
**Payload** _(sobriété B5 2026-05-04 — `side_effects` retiré, le TMS calcule sa propre logique sur le diff)_ : diff JSON avec champs modifiés uniquement (PATCH partiel) :
...
Licence MTS-1 terminée _(état final post-V2)_. Plus de fallback système. Si TMS Savr indisponible > 30 min :
...
- **5xx / timeout (Pennylane down)** : `factures.statut = en_attente_pennylane`, retry automatique **3 paliers : 5 min → 1h → 24h** _(simplifié 2026-05-08 — ex-5 paliers 5 min/30 min/2h/6h/24h, retry policy unifiée avec celle des webhooks Plateforme↔TMS §1)_. Si les 3 tentatives échouent → notification Admin urgente (email + bandeau orange sur fiche facture, pas de widget dashboard dédié). Retry manuel Admin via bouton "Renvoyer" sur la fiche facture. Email client envoyé uniquement après succès. Voir [[06 - Fonctionnalités détaillées/08 - Génération et édition facture (Admin)#2. Synchro Pennylane — flux unique (nominal + erreurs + retry)]].

---

## 3. API Plateforme ↔ Everest (A Toutes!)

### V1 — appel direct Plateforme → Everest _(actualisation 2026-05-09)_
...
| ----------------------------------------------------------------------- | -------------------------------------------- | ---------- | ---------------------------------- |
| `ag_velo_express`                                                       | Vélo cargo express (< 90 min avant collecte) | **75**     | A Toutes! vélo last-minute         |
...
**Spec technique V1 (à figer avant dev)** : endpoints Everest pour services 71 / 75 / 91, schéma d'authentification (clé API + secret stockés dans Supabase Vault), payload (lieu, association destination, créneau, volume estimé), gestion d'erreur (rejet synchrone HTTP vs asynchrone webhook), retry policy aligné §08 (3 paliers 5 min / 1h / 24h). À spécifier dans une session dédiée Everest API V1 avant le dev.

### V2 — intégration Everest déplacée vers le Savr TMS _(décision 2026-04-21)_
...
## 3bis. API Plateforme ↔ MTS-1 _(ajout 2026-05-29 ; flux sortant + URLs réconciliés sur le relevé as-built Bubble 2026-06-06)_
...
### 3bis.10 Réconciliation / archive _(différé V2 — revue sobriété §08 App 2026-05-31 A2)_
...
3. **Fermée 2026-06-06 par le relevé as-built Bubble** : le flux réel est **`POST /v3/customerOrders` → `POST /v3/tours` (DRAFT) → `POST /v3/tours/{tourId}/dispatch` (`carrierShareableCode`) → `PUT /v3/tours/{tourId}/validate`**. 1 collecte = 1 commande + 1 tournée. Pas de `delegate`, pas d'`import`. _(Confirmation éditeur facultative — le comportement Bubble en prod fait foi.)_
...
- `id`, `destinataire_user_id`, `destinataire_email`, `template_slug`, `objet`, `variables_jsonb`, `resend_id`, `statut` (`envoye`/`ouvert`/`clique`/`bounce`/**`echec`** — _valeur ajoutée 2026-06-07, F3_), `tentative_numero`, `created_at`, `delivered_at`
...
### Gestion des échecs d'envoi _(spécifié 2026-06-07 — F3 tranchée Val, session test-scenarios §06.02)_
...
- 5xx ou timeout → retry **3 fois : 5 min / 1h / 24h** _(simplifié revue sobriété Bloc B 2026-05-01 B1 — ex-5 fois 5 min/30 min/2h/6h/24h)_
- Backoff exponentiel avec jitter aléatoire ± 10%
- Échec après les 3 retries (4 tentatives au total) → notification Admin Savr + statut `echec_final` dans `integrations_logs` _(corrigé revue sobriété §08 App 2026-05-31 C1 — ex « après 5 tentatives », résidu de l'ex-politique 5 paliers)_
- Resume manuel par Admin (bouton "rejouer la sync")

> **Source de vérité retry** _(revue sobriété §08 App 2026-05-31 C2)_ : cette section §6 est la **définition authoritative** de la retry policy Plateforme (3 paliers 5 min / 1h / 24h). Les sections §1 (TMS), §2 (Pennylane) et §3bis.9 (MTS-1) y renvoient et ne la redéfinissent pas — éviter la dérive (cf. bug C1 ci-dessus, né d'une définition dupliquée).
...
## 9. Endpoints Admin — Paramètres Taux de recyclage _(ajout 2026-05-06)_
...
```json
{
  "filieres": [
    ...
    { "code_filiere": "carton", "taux_captation": 0.9, "...": "..." },
...
## 9ter. Endpoints Admin — Facteurs CO₂ _(ajout 2026-06-04, Sujet 3)_
...
### 9ter.5 Facteur CO₂ AG (repas donnés) _(ajout 2026-06-04 bis)_
...
## 9bis. Endpoints Admin — Coefficient de perte labo _(ajout 2026-05-22)_
...
## 10. Endpoints Admin — Dispatch manuel & Tableau revenus _(ajout 2026-05-07)_
...
**Auth** : `admin_savr` ou `ops_savr` (les deux peuvent renvoyer le dispatch) _(« S7 » → « dispatch » 2026-06-07 F5 — S7 = webhook TMS→App `plaque-saisie`, sans rapport avec ce flux sortant)_.
...
- Si `prestataire_id` non fourni : branche selon `collectes.statut_tms` _(tranchée F3 2026-06-07)_ :
...
```json
{
  ...
  "totaux_periode": {
    "nb_zd_total": 350,
    "montant_zd_total": 124000.0,
    "nb_ag_total": 180,
    "montant_ag_total": 54000.0
  }
}
```
...
### 11. RLS extension `ops_savr` sur back-office _(ajout 2026-05-07)_
...
| `evenements`                                                      | ALL               | ALL                                                                                                                                                                                                        | —                 |
| `factures`                                                        | ALL               | UPDATE statut (valider, renvoyer Pennylane) ; **PAS** d'édition lignes, **PAS** d'annulation, **PAS** d'avoir _(refonte 2026-05-08 — "relance facture" retirée : les relances sont gérées dans Pennylane)_ | —                 |
...
- **Event-driven par défaut** _(polling supprimé revue sobriété 2026-05-01 Bloc A A4 — retry 3 paliers + dédup `integrations_inbox` 7j couvrent les pannes <24h, intervention manuelle au-delà)_
...
- **Retry policy uniforme TMS** _(3 paliers depuis 2026-05-01 Bloc B B1 — ex-5 paliers)_ : 5 min / 1h / 24h (alignement avec §08 TMS) — 2026-04-22 / 2026-05-01
