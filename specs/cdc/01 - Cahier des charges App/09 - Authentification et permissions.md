...
- **Champs admin/ops only Plateforme** *(ajout 2026-05-08)* : 4 colonnes (`commentaire_lieu`, `siren`, `email_gestionnaire`, `reference_citeo`) **strictement réservées** à `admin_savr` + `ops_savr` Plateforme en lecture comme en écriture. `app_domain() = 'tms'` n'a **aucun accès** (ni SELECT ni UPDATE). Voir `R_lieux_admin_only_fields` §05 pour le détail des GRANT et la vue `v_lieux_clients`.
...
- `lieux_plateforme_full_write_preserved` : `admin_savr` UPDATE toutes colonnes `plateforme.lieux` → OK (comportement historique). **⚠ À vérifier sous `service_role`, PAS sous `authenticated` (révision 2026-09-21)** : depuis la migration `20260921210000`, l'écriture PostgREST directe de `plateforme.lieux` est fermée à `authenticated` (REVOKE table-level INSERT/UPDATE/DELETE), y compris pour un JWT `admin_savr` — les policies `lieux_admin` (`FOR ALL`) et `lieux_ops_write` subsistent mais sont inertes. Ce que ce scénario garantit est le périmètre de COLONNES (toutes, par opposition aux 2 colonnes logistiques d'`admin_tms` en `lieux_tms_write_only_2_logistic_cols`), pas le canal d'écriture. Le canal est celui déjà posé par `lieux_admin_only_fields_hidden_from_clients` : « Staff lit/écrit via `service_role` ».
- **`lieux_admin_only_fields_hidden_from_clients`** *(ajout 2026-05-08, révisé 2026-06-12, corrigé 2026-06-17)* : un user `traiteur_*` / `agence_*` / `gestionnaire_*` / `client_organisateur_*` SELECT `plateforme.lieux.commentaire_lieu` (ou `siren`/`email_gestionnaire`/`reference_citeo`/`commentaires_internes`) → doit échouer. **Implémentation V1 obligatoire (décision Val 2026-06-12 — restriction effective au go-live)** : vue `v_lieux_clients` **SECURITY INVOKER** (la RLS `lieux_clients_select` filtre les lignes par appelant ; le masquage colonne est garanti par `REVOKE SELECT` table-level + `GRANT SELECT (whitelist 22 colonnes)` sur `plateforme.lieux`, pattern identique au masquage F5 `factures`). Colonnes masquées : `commentaire_lieu`, `siren`, `email_gestionnaire`, `reference_citeo` (4 admin-only) **+ `commentaires_internes`**. Staff lit/écrit via `service_role`, non impacté. **Test P1 bloquant CI.**
...
>
> **Note F5 lot ⑨ (2026-06-07, tranché Val)** : la lecture du référentiel traiteurs par `agence` + `gestionnaire_lieux` (combobox « Traiteur opérant » §06.01, nom du traiteur opérationnel sur la fiche collecte §06.11) passe par la **vue whitelist `v_referentiel_traiteurs`** (`id`, `nom` **seuls** ; `nom` = libellé unique `COALESCE(NULLIF(nom,''), raison_sociale)` ; `type='traiteur' AND est_shadow=false` ; SECURITY DEFINER ; GRANT SELECT à `authenticated` — cf. [[04 - Data Model]]). **Restriction 2026-09-22** (revue sécurité PR #363) : `raison_sociale` **retirée de la projection** — elle exposait la raison sociale de tous les traiteurs actifs à tout utilisateur connecté, au-delà du nom autorisé par §06.05 / §06.11 §3. La matrice SELECT ci-dessus reste inchangée (aucun élargissement du SELECT direct). Exclusion registre agence câblée dans `v_registre_dechets` (prédicat rôle ≠ 'agence', F6 même session). pgTAP : `referentiel_traiteurs_whitelist_ok` + `registre_agence_denied` (P1).
>
...
| traiteur_manager | `organisation_id = auth.jwt()->>'organisation_id'` | `organisation_id = auth.jwt()->>'organisation_id'` (invitation commerciaux) | `organisation_id = auth.jwt()->>'organisation_id'` ; garde DB : ni auto-désactivation (2026-09-03), ni auto-changement de rôle (2026-09-21, `trg_users_block_role_escalation` volet 3) | — |
...
> ⚠ **Depuis le 2026-09-23 (migration `20260923150000`), `authenticated` n'a plus les privilèges `INSERT` ni `DELETE` sur `plateforme.factures`** (REVOKE table-level, sans re-GRANT — arbitrage Val). Les policies `fac_admin`, `fac_client_select`, `fac_ops_select` et `fac_ops_update` sont **conservées** : un appel PostgREST direct en création ou en suppression lève `42501` (insufficient_privilege) avant l'évaluation RLS. La matrice ci-dessus se lit donc **au niveau `service_role`** pour l'INSERT et le DELETE. Toute création ou suppression de facture passe par les routes API Next sous `service_role`, seules à attribuer le numéro par `f_attribuer_numero_facture` (séquence gapless fiscale), à poser les invariants de `lib/facturation/*` et à tracer `audit_log` (aucun trigger d'audit sur cette table).
>
> ⚠ **L'`UPDATE` n'est PAS retiré** (arbitrage Val 2026-09-22). La capacité d'écriture d'`ops_savr` sur les factures n'est pas posée par la matrice ci-dessus — qui ne comporte aucune ligne `ops_savr` — mais par la **matrice étendue `ops_savr`** (§3, « source de vérité unique des permissions `ops_savr` »), et elle y est PRÉCISE : « Valider + envoyer Pennylane » est accordé, tandis que « Éditer ligne / montant » et « Annuler / Générer avoir » lui sont explicitement **refusés**. **Ne pas résumer cela en « ops_savr écrit les factures »** : le raccourci a déjà produit une survente lors du lot `20260923150000`, corrigée en revue.
>
> Cette frontière est tenue par **deux mécanismes distincts**, ce qu'il faut savoir avant de toucher à l'un d'eux :
>
> | Capacité refusée à `ops_savr` | Mécanisme réel |
> |---|---|
> | « montant » | trigger `trg_ops_immutable_cols` (4 colonnes de montant : `montant_ht`, `montant_tva`, `montant_ttc`, `taux_tva`) |
> | « Éditer ligne » | absence de policy d'écriture ops sur `plateforme.factures_collectes` |
> | « Annuler » | trigger `trg_ops_block_facture_annulation` (garde de **valeur**, pas d'immuabilité de colonne) |
> | « Générer avoir » | absence de policy INSERT pour ops sur `plateforme.factures` (un avoir est un INSERT) |
>
> Les 2 triggers R10b portés par `factures` (`20260629120000`) ne couvrent donc que les deux premières lignes — « 4 » est le nombre de **colonnes** gardées, pas de triggers. `fac_ops_update` reste pleinement active sous `authenticated`, et `fac_admin` (FOR ALL) aussi. La fermeture de l'`UPDATE` fera l'objet d'un **lot dédié** — ne pas la refermer « par symétrie » avec le reste de la série « écriture client fermée », et ne pas attendre d'elle qu'elle ferme les deux moitiés tenues par la RLS.
>
> ⚠ **Résiduel assumé, à nommer tel quel.** Les 2 triggers R10b ne gardent QUE `ops_savr` : leurs fonctions sortent si `f_app_role()` n'est pas `ops_savr`. Sous JWT `admin_savr`, l'UPDATE PostgREST direct reste **non borné et non tracé** — mesuré 2026-09-23 : renumérotation d'une facture `emise`, passage de `numero_facture` à `NULL` et modification de `montant_ht` aboutissent tous, sans ligne d'`audit_log`. L'`UNIQUE` interdit un doublon de numéro, pas une renumérotation : la numérotation gapless reste corruptible par UPDATE direct.
>
> ⚠ La ligne `admin_savr … DELETE : — (pas de suppression, uniquement avoirs)` n'était pas vraie avant ce REVOKE : `fac_admin` étant `FOR ALL` (`polcmd = '*'`), le DELETE direct aboutissait sous JWT `admin_savr` (mesuré 2026-09-22 : `DELETE 1`). Elle l'est désormais par construction.

> **Masquage colonnes sensibles (décision F5 test-scenarios §06.08 2026-06-07)** : `factures.marge_logistique` (marge Savr, écrite par le trigger cross-schema `fn_recalc_marge_tournee`) + `erreur_synchro*` ne doivent **jamais** être lisibles par les rôles clients — la RLS row-level ne masquant pas une colonne, les rôles clients (manager, commercial, agence, gestionnaire_lieux) lisent via la **vue whitelist `v_factures_client`** (`SECURITY INVOKER` — s'exécute avec les droits de l'appelant, **⚠ nécessite une policy SELECT explicite sur `plateforme.factures`** pour les rôles `traiteur_manager`, `traiteur_commercial`, `agence`, `gestionnaire_lieux`, restreinte à `organisation_id = (auth.jwt()->>'organisation_id')::uuid` — SANS cette policy, RLS DENY ALL rend la vue vide pour tous les rôles clients ; bug détecté M3.5 2026-06-16, policy `fac_client_select` ajoutée migration M3.5) ; le SELECT direct sur `plateforme.factures` est limité staff (`admin_savr`/`ops_savr`). **⚠ État réel au 2026-09-22 — le masquage est obtenu autrement que décrit.** La migration `20260616120000` a retiré le `SELECT` **table-level** à `authenticated` puis l'a ré-accordé sur une **liste blanche de 26 colonnes**, ce qui masque `marge_logistique`, `erreur_synchro`, `erreur_synchro_at`, `derniere_tentative_pennylane_at`, `pennylane_statut` et `pennylane_push_at`. Les **5** chemins de lecture client lisent donc la **table en direct** et non la vue (`api/v1/traiteur/factures`, `api/v1/agence/factures`, `api/v1/gestionnaire/mon-organisation/factures`, `(agence)/agence/mon-organisation`, export CSV `lib/exports/builders.ts`) ; `v_factures_client` existe et reste exploitable, mais aucun code ne l'appelle. **Ne JAMAIS ajouter un `GRANT SELECT` table-level sur `plateforme.factures`** : il ré-exposerait les 6 colonnes closes. pgTAP : `test_factures_marge_invisible_clients` (deny SELECT table direct manager + vue sans colonne) + **`test_factures_vue_client_non_vide_manager`** (exercer `v_factures_client` sous `traiteur_manager` → liste non vide, pas d'erreur RLS) — P1 bloquants CI. Cf. [[04 - Data Model#Table : `factures`]].
  ...
  SELECT CASE p_entity_type
    ...
    WHEN 'plateforme.rapports_rse'    THEN plateforme.f_collecte_visible(
           ...
           /* M3.4 2026-06-17 — aligné f_collecte_visible (même pattern que bordereaux_savr/attestations_don).
              ...
              collecte_id non NULL pour tous les rapports V1 (batch J+1) ; cas NULL → false (deny-safe). */
...
v_attributions_gest_role_non_gestionnaire_denied v_attributions_gestionnaire client_organisateur SELECT FAIL (organisation RATTACHÉE au lieu via organisations_lieux — garde de rôle D1 ; contrôle de non-vacuité : même rattachement, rôle gestionnaire_lieux → ≥ 1 ligne)
...
## 3quinquies. Contrat d'erreur des RPC `SECURITY DEFINER` *(ajout 2026-09-23 — arbitrage Val, divergence SECU-RLS `benchmark-gardes-role-fail-closed`)*

Toute fonction `SECURITY DEFINER` exposée par PostgREST court-circuite la RLS de l'appelant : son contrôle d'accès est **entièrement dans son corps**. Les trois règles ci-dessous sont opposables à toute fonction de ce type (famille `f_benchmark_*` en premier lieu), et à tout lot qui y touchera.
...
- **Extension transactionnelle agence + gestionnaire_lieux (2026-05-07)** : INSERT/UPDATE `evenements` + `collectes` ouvert aux 2 rôles. Périmètre agence = ouvert. Périmètre gestionnaire = fermé via `organisations_lieux` + filtre traiteur opérationnel non-shadow. Pas de nouveau rôle créé (extension périmètre des rôles existants). **⚠ Révisé 2026-09-15 : l'INSERT/UPDATE direct sur `collectes` via PostgREST est fermé pour tous les rôles clients (REVOKE table-level, migration `20260915160000`) — les policies `col_insert` / `col_update_client` / `col_update_commercial` subsistent mais sont inertes (42501 avant RLS). Toute écriture passe par les routes API sous `service_role`. **Même fermeture pour `evenements` (REVOKE table-level, migration `20260915190000`, PR #328, appliquée dev + prod) : les policies d'écriture client sur `evenements` subsistent mais sont inertes (42501 avant RLS). Même fermeture pour `lieux` (REVOKE table-level INSERT/UPDATE/DELETE, migration `20260921210000`, 2026-09-21) — avec une nuance : sur `lieux` il n'existait aucune policy d'écriture CLIENT, les deux policies ouvrantes étaient STAFF (`lieux_admin` en `FOR ALL`, `lieux_ops_write` en UPDATE) et elles étaient réellement ATTEIGNABLES sous `authenticated` avec un JWT `admin_savr`/`ops_savr` porté par la clé anon. Le SELECT colonne-level (liste blanche de 23 colonnes, `20260617170000`) n'est pas touché.**
