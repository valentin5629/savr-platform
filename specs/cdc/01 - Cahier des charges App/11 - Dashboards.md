...
| Carte | Source |
|-------|--------|
| Collectes non transmises au TMS (split ZD / AG) *(renommée Sujet 2 2026-05-26, ex « à valider » — monitoring d'échec E1, pas de gate Admin)* | `collectes` `statut_tms = 'non_envoye'` ET `tms_reference IS NULL` ET **`statut IN ('programmee','validee')`** *(**définition canonique de « à dispatcher », tranchée Val 2026-09-14** — remplace `statut = 'programmee'` seul. Une collecte validée transporteur mais non transmise au TMS **est** à dispatcher : c'est le cas d'usage le plus utile, et le sous-libellé « validées transporteur » de la tuile le disait déjà. **Ce prédicat est unique et partagé** par la tuile KPI (`/api/v1/admin/collectes/chip-counts`), le chip « Non transmises ZD/AG » (`lib/collectes-chips.ts`), le sous-libellé de la tuile et le bouton d'action rapide « Dispatcher » des cartes (`aDispatcherZd`) — aucune de ces 4 surfaces ne doit en dévier.)* |
...
- **Histogramme** *(refonte revue E2E 2026-07-18 : plus de période propre — **piloté par le filtre commun du Bloc 2**, côte à côte avec le tableau en 50/50 ≥ `lg`. Le libellé « 12 mois glissants » n'est exact qu'au défaut.)* : barres empilées par mois (segment ZD + segment AG), toggle "nombre" / "montant HT". Source nb : `collectes.date_collecte`. **Montant ZD (tranché 2026-06-07, F5 lot ⑫)** : factures `statut IN ('emise','payee')` par `date_emission` (brouillons exclus, cohérent R_revenus_imputation_organisation), **avoirs ZD comptés en négatif** sur leur mois d'émission. **Montant AG (patch divergence M3.5, arbitrage Val 2026-07-07)** : **CA économique** = Σ collectes AG livrées (`statut IN ('realisee','cloturee')`) × coût par collecte du pack (`packs_antgaspi.prix_unitaire_ht` ≈ `montant_total_ht` / crédits initiaux), imputé au mois de `date_collecte`. Les factures `achat_pack_antigaspi` **et leurs avoirs** ne sont **plus** reflétés dans le CA AG de pilotage (revenu comptable = module Facturation seul, pas de double comptage — avoirs pack exclus, tranché Val 2026-07-07).
- **Tableau "Revenus par organisation"** : période = **filtre commun du Bloc 2** (`date_collecte` from/to, **défaut 12 derniers mois glissants alignés au 1er du mois** — décision Val 2026-07-18, aligné §8 l.180 / §06.04 / §06.05 ; remplace l'ancien défaut « mois en cours », qui était la seule valeur spéciale du chapitre). **Ni presets de période ni bouton Export CSV sur le Dashboard Admin** (retirés revue E2E 2026-07-18 ; l'endpoint `?format=csv` est conservé, sans déclencheur UI ; les presets restent en place sur les autres dashboards). Colonnes : nom, type, nb ZD, montant ZD HT (imputation factures), nb AG, **montant AG HT = CA économique** (coût/collecte pack × collectes AG livrées, cf. règle histogramme ci-dessus). Imputation = organisation programmatrice (`evenements.organisation_id`), cf. [[05 - Règles métier#R_revenus_imputation_organisation]]. Tri défaut `montant_total_desc`. Pagination 50/page. Export CSV.
...
- **Blocs ZD** : KPI **(5 cartes — Nb collectes / Tonnage / Taux de recyclage / kg/pax / CO₂ évité)** *(« Marge générée » retirée de la rangée au profit de « CO₂ évité » — décision Val 2026-07-13 ; formule et endpoint conservés)* · Bloc 2 ZD évolution mensuelle (barres empilées 5 flux) · Bloc 3 ZD jauges kg/pax × benchmark parc (5 jauges, k-anonymat ≥ 5 collectes **et ≥ 3 acteurs distincts**) · Bloc 4 ZD donut répartition tonnages · Bloc 5 ZD prochaines collectes ZD · Bloc 6 ZD top 5 lieux ZD · Bloc 7 ZD top 5 commerciaux ZD · Bloc 8 ZD bouton export synthèse PDF (type=ZD).
- **Blocs AG** : KPI (5 cartes — Nb collectes / Repas donnés / Pax cumulés / Repas/pax / CO₂ évité) *(carte « CO₂ évité » ajoutée — décision Val 2026-07-13 ; CO₂ AG « évité seul V1 », modale méthode par repas)* · Bloc 2 AG évolution mensuelle (courbe) · Bloc 3 AG top associations bénéficiaires · Bloc 4 AG **Mon pack AG** (pleine largeur, pack actif unique, demande renouvellement + badges orange/rouge) · Bloc 5 AG prochaines collectes AG · Bloc 6 AG top 5 lieux AG · Bloc 7 AG top 5 commerciaux AG · Bloc 8 AG bouton export synthèse PDF (type=AG).
...
- **Cartes KPI non cliquables vers la liste Collectes** *(décision Val GO-VISUAL 2026-07-10 — revient sur refonte 2026-05-07 / BL-P2-11/43)*. Seule exception : la carte **CO₂ évité** ouvre une **modale « Détail de l'impact carbone »** (pas de navigation). Sparkline + variation N-1 par carte (sauf kg/pax). **Drill-down** : les lignes des Top 5 lieux / commerciaux / associations (Blocs 3 AG, 6, 7) sont cliquables → liste Collectes filtrée (décision Val 2026-07-14).
...
- **Cartes KPI non cliquables** *(décision Val GO-VISUAL 2026-07-10)*. **Correction 2026-07-14** : la page Collectes gestionnaire **existe** (nav §06.05 §1 + section « Liste Collectes ») — l'affirmation « page Collectes supprimée » (modèle Événements, refonte 2026-05-03) est périmée. Drill-down Top 5 lieux / traiteurs (Blocs 6/7) → liste Collectes filtrée (décision Val 2026-07-14).
