...
**Statut** : Draft V1 (création 2026-05-07 — extension programmation 3 types)
**Dernière mise à jour** : 2026-06-07 bis (**Complément session test-scenarios lot ⑨** — 2 floues résiduelles tranchées Val : **F5** lecture référentiel traiteurs = **vue whitelist `v_referentiel_traiteurs`** (id/nom/raison_sociale, SECURITY DEFINER, agence + gestionnaire — combobox « Traiteur opérant » et nom fiche collecte étaient morts RLS, `organisations` SELECT self only) ; **F6** exclusion registre agence = **prédicat `rôle ≠ 'agence'` dans `v_registre_dechets`** (la vue scopait par `organisation_id`, l'agence y aurait vu ses lignes). Décision F1 (users self-only, Top 5 retiré) **reconfirmée Val — ne pas re-proposer org-wide**. Propagé : §04 (2 vues) + §09 (note F5) + tests.) · Antérieur même jour : (**Session test-scenarios lot ⑨ (skill `cdc-test-scenarios`)** — 4 floues tranchées Val : F1 Top 5 commerciaux + bloc Utilisateurs retirés V1 (divergence forcée #8, RLS `users` agence = self only) ; F2 complétion SIRET shadow = RPC `f_completer_siret_shadow` SECURITY DEFINER ; F3 notifs Admin shadow in-app seules ; F4 trigger `trg_cerfa_debloque_siret` finalisation auto bordereaux brouillon. Propagé : §09 matrice users + organisations, §04 notes shadow, §11 Dashboards §4. Scénarios : `tests/06.11-espace-agence-scenarios.md`.) · Précédent : 2026-06-03 (**Revue de sobriété §06.11 (skill `cdc-review-sobriete`)** — bascule en **réplique stricte du §06.04**. Sur arbitrage Val : (A1) workflow shadow conservé — collecte programmable immédiatement avec traiteur hors référentiel, champs non tous obligatoires, Admin notifié en info-only, **aucune validation requise** pour que la collecte ait lieu. (B1+B2) **parité absolue §06.04 en V1** : suppression de ~250 lignes de re-description redondante des blocs dashboard, suppression des sections nav « Événements » / « Rapports RSE » / « Mon pack AG » (fondues comme §06.04), retrait des divergences agence du dashboard (filtre + bloc « Top 5 traiteurs opérationnels », 5e dimension benchmark). Seules différences forcées conservées : périmètre RLS donneur d'ordre, traiteur opérationnel affiché sur la fiche collecte, workflow shadow, pas de registre, branding agence sur PDF (règle §12). KPI Marge ZD reste exclu (4 cartes ZD — décision 2026-05-07 maintenue). Propagé : §11 Dashboards §4, §12 Reporting §1.6, §00 Index App, §00 Index section 06. Cross-CDC 0.)
...
| 4 | **Workflow shadow à la programmation** | Voir §06.01 §Cas Agence. La collecte est **programmable immédiatement** avec un traiteur hors référentiel ; tous les champs ne sont pas obligatoires (Nom commercial + Raison sociale obligatoires, SIRET fortement recommandé non bloquant, Ville optionnelle) ; l'Admin Savr est **notifié en info-only** ; **aucune validation Admin n'est requise pour que la collecte ait lieu**. Seul le **bordereau Cerfa** reste en `brouillon` tant que le SIRET du traiteur shadow n'est pas renseigné. |
...
- **F4** : trigger **`trg_cerfa_debloque_siret`** `AFTER UPDATE OF siret ON organisations` (scope `est_shadow=true`, siret NULL→NOT NULL) → les bordereaux Cerfa en `brouillon` liés aux collectes de cette fiche shadow sont **finalisés automatiquement** (zéro action humaine).
...
CREATE POLICY agence_select_collectes ON plateforme.collectes
  FOR SELECT
  USING (
    EXISTS (
...
| **Déblocage Cerfa auto au SIRET (F4, 2026-06-07)** | Régénération manuelle Admin | Trigger `trg_cerfa_debloque_siret` AFTER UPDATE OF siret → bordereaux brouillon finalisés. Zéro action humaine. |
