...
**Dernière mise à jour** : 2026-07-06 (**patch revue adversariale concurrence, arbitrages Val 2026-07-06** — trigger dérivation R6.1 sérialisé PAR COLLECTE (`FOR UPDATE`, ordre déterministe) ; **NOUVELLE table `tms.outbox_events`** (webhooks sortants S1-S11 transactionnels, worker lease/claim, seq — DDL cible 92→93, pglast 320 stmts) ; **NOUVELLE colonne `collecte_tournees.statut_execution`** (`a_faire`/`faite`/`incident`, gate clôture M05 E4) ; NOUVEAUX triggers `trg_tournees_transitions` (matrice R6.2 whitelist) + `trg_tournees_horaires_verrouilles` (horaires figés si `cout_final_verrouille`) + `trg_pesee_tardive_s5_correction` (S5 correction auto) ; RPC `m04_evaluer_completude` (FOR UPDATE) ; `pesees` : fusion addendum M05 dans le bloc canonique (`idempotency_key` UNIQUE **sans default**, `source`, `tare_override_motif`, écriture `ON CONFLICT`) + attribution `ordre_pesee` serveur sous lock) / 2026-06-04 (**propagation Bloc 3 — workflow RGPD géoloc** — ajout colonne `users_tms.consentements jsonb` (trace acceptation notice d'information géoloc à l'inscription, base légale intérêt légitime)) / 2026-05-29 (**propagation généralisation multi-vélo AG V2** — `everest_missions.collecte_tms_id` non-unique (N missions/collecte en multi-vélo, distinction par `tournee_id`) ; helper `tms.m14_lookup_mission_by_collecte` passé en `SETOF tms.everest_missions` ; trigger `trg_m14_cascade_cancel` itère sur les N missions actives (un `pg_notify` par mission) + lookup corrigé via `collecte_tournees` (ex `collectes_tms.tournee_id` retiré)) / 2026-05-03 (**propagation refonte formulaire §06.01 Plateforme** — renommage `tms.collectes_tms.plaque_requise` → `controle_acces_requis` (flag unique plaque + nom chauffeur), trigger `trg_validate_tournee_plaque_requise` → `trg_validate_tournee_controle_acces` (validation étendue à `chauffeur_id IS NOT NULL`), webhook S7 enrichi payload `plaque + chauffeur_nom`) / 2026-04-30 (**propagation sobriété M13 Blocs C+D** — `marathon_webhook_signing_key` struck through seed `secrets_metadata` V1 (D1 : spec webhook Marathon non confirmée)) / 2026-04-25 (**propagation M14 Intégration Everest** — `everest_missions` enum `statut_everest` élargi à 10 valeurs (+`creation_failed`/`created_manually`/`completed_incomplete`/`cancelled_externally`) + 4 colonnes `manual_acceptance_*` + 1 CHECK constraint + 1 index, `collectes_tms` colonne `everest_mission_id` + index, 2 secrets seed Vault `secrets_metadata` (`everest_webhook_token` + optionnel `everest_access_token`), 6 paramètres `m14_*` namespace, 1 trigger DB `trg_m14_cascade_cancel`, 1 fonction SQL helper `m14_lookup_mission_by_collecte` / **propagation M13 Administration TMS** — 4 tables nouvelles `users_tms_devices_trusted` / `alertes_codes_overrides` / `secrets_metadata` / `impersonation_sessions`, 2 colonnes ajoutées sur `parametres_tms` (`requires_redeploy`, `deprecated`), 4 colonnes ajoutées sur `users_tms` (`desactivee_at`, `desactivee_par_user_id`, `raison_desactivation`, `mfa_active`), `m11_slack_webhook_url` marqué `deprecated=true` à migrer vers Vault, 17 paramètres `m13_*` à seeder, 10 codes alertes `m13_*` à seeder catalogue M11) / 2026-04-25 propagation M10 Gestion exutoires Veolia — `stocks_bacs_entrepot` 3 nouvelles colonnes `capacite_max`/`seuil_saturation_pleins`/`quantite_pleine_recomptee`, `passages_veolia` 2 nouvelles colonnes `cree_par_action`/`stock_update_done` + renommage `nb_bacs_enlevés` → `nb_bacs_enleves`, nouvelle table append-only `recomptages_stocks_entrepot_log`, 5 paramètres `m10_*`, 3 fonctions SQL + 5 triggers DB + 2 crons pg_cron, 9 codes alertes M10 à seed `alertes_catalogue`) / 2026-04-24 (propagation M11 Alerting transverse — 3 types enum, tables `alertes_catalogue` + `alertes` + `alertes_evenements_log`, 12 paramètres `m11_*`, 8 fonctions SQL + 5 crons pg_cron / propagation M03 Portail prestataire self-service — enrichissement `types_vehicules` 4 colonnes + update seed V1, nouvelle colonne `collectes_tms.plaque_requise` → `controle_acces_requis` (renommage 2026-05-03), fonction SQL `merger_type_vehicule` / propagation M05 App mobile chauffeur — enrichissement `pesees` offline/source/override, seed `sans_contenant` dans `types_contenants`, enum `incidents` enrichi, table `auth_sessions_tms` pour device binding + propagation M04 — présomption 0kg déplacée vers clôture collecte R_M05.18)
...
BEGIN
  ...
  -- 2.0 EXCEPTION FILET (2026-07-06 COH-05, arbitrage RC-M04-02 iii) : tournée filet JAMAIS démarrée
  -- (clôture auto trg_collectes_autoclose_filet : OLD.statut='acceptee', horaires NULL, toutes les
  -- collectes liées terminales par incident/annulation) → coût 0 €, statut financier 'calcule',
  -- PAS d'émission m07_horaires_manquants (rien à corriger : aucune vacation, aucun horaire attendu).
  IF NEW.heure_reelle_debut IS NULL AND NEW.heure_reelle_fin IS NULL
     AND OLD.statut = 'acceptee'
     AND NOT EXISTS (
       SELECT 1 FROM tms.collecte_tournees ct
       JOIN tms.collectes_tms c ON c.id = ct.collecte_tms_id
       WHERE ct.tournee_id = NEW.id
         AND c.statut_operationnel NOT IN ('incident','annulee')
     ) THEN
    UPDATE tms.tournees
       SET cout_calcule_ht    = 0,
           cout_detail        = jsonb_build_object('raison', 'filet_jamais_demarree'),
...
- `m07_horaires_manquants` (critical, M07) : précheck step 2 KO → manque heure_reelle_debut/fin. **Non émise pour une tournée filet jamais démarrée** (step 2.0, exception COH-05 2026-07-06 : coût 0 €, `statut_financier='calcule'`)
...
BEGIN
  IF (OLD.statut IS DISTINCT FROM NEW.statut AND NEW.statut = 'terminee') THEN
    ...
    LOOP
      ...
      -- Émission S5 terminal unique (COH-01, arbitrage RC-M04-06) : INSERT outbox dans la MÊME
      -- transaction que la dérivation — le worker §08 §2bis livre le webhook, jamais le trigger.
      IF FOUND THEN
        INSERT INTO tms.outbox_events (event_id, event_type, aggregate_type, aggregate_id, payload)
        VALUES (
          gen_random_uuid(),
          'collecte-terminee',              -- S5 terminal unique
          'collecte',
          v_collecte_id,
          tms.fn_build_payload_s5(v_collecte_id, 'terminal')  -- enveloppe complète §08 (pesées des N camions
                                                              -- sommées par (collecte_tms_id, flux), don_alimentaire AG)
        );
      END IF;
    END LOOP;
  END IF;
...
| `trg_collectes_autoclose_filet` | AFTER UPDATE OF statut_operationnel ON `collectes_tms` | **Filet auto-close R6.2 (catalogué 2026-07-06 COH-06, décision RC-M04-02)** : si NEW passe à un statut terminal **non-`realisee`** (`incident` / `annulee` — terminaux d'incident/annulation de `statut_operationnel`, cf. R6.1 §05), pour chaque tournée liée non terminale : si **toutes** ses `collectes_tms` (via `collecte_tournees`) sont terminales par incident/annulation → clôture gardée `UPDATE tournees SET statut='terminee' WHERE id=… AND statut IN ('en_cours','acceptee')` (0 ligne = no-op ; la transition `acceptee→terminee` est dans la whitelist `trg_tournees_transitions`). Tournée filet jamais démarrée → cas coût 0 € du précheck M07 (cf. exception step 2). Hors DDL diffable, pas de regel |
...
**Ordre d'exécution RPC ↔ trigger (précisé 2026-07-06 COH-20, demandé par RC-M04-05)** : `trg_validate_tournee_controle_acces` s'évalue **dans la même transaction** que la bascule atomique de `tms.m04_evaluer_completude` (après le `FOR UPDATE`, au moment des UPDATE) ; s'il RAISE (plaque/chauffeur manquants), **la bascule entière rollback** — aucun état à moitié basculé (ni collectes `en_attente_execution` sans tournée `acceptee`, ni l'inverse).

**Code alerte requis (à seeder dans `alertes_catalogue` — catalogue M11 §9)** : `m04_cloture_reelle_post_forcee` (**warning**, M04 W9 replay — criticité alignée 2026-07-06 COH-07 : l'enum `alerte_criticite` ne porte pas `info`, retiré sobriété Bloc 3 A1).
