...
14. **Décision annulée — saisie plaque chauffeur retirée V1 (propagation M05 2026-06-04, arbitrage Val).** La plaque pour contrôle d'accès / registre est la plaque pré-saisie manager (M03 E4, webhook S7). *(Propagation CDC Plateforme historique conservée : suppression email client T+3h V1, template `plaque_chauffeur` retiré, `collectes.recevoir_plaque_chauffeur` + `collectes.email_plaque_envoye_at` supprimés.)* **Réintroduction V1 (2026-07-15, divergence PLAQUES) :** la suppression V1 ci-dessus concernait l'ancien template `plaque_chauffeur` (notification T+3h à la saisie plaque). Un **nouveau** workflow « infos accès » le remplace — nom + téléphone chauffeur + accompagnant, par collecte `controle_acces_requis`, **saisie Admin en V1**, email récap au programmateur à complétude (nouveau stamp `collectes.infos_acces_email_envoye_at`, sans réintroduire `recevoir_plaque_chauffeur` / `email_plaque_envoye_at`).
