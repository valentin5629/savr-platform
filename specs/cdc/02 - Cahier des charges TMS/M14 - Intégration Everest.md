...
**Implémentation** : trigger DB `trg_m14_push_mission` AFTER UPDATE → enqueue job `m14_create_mission` (worker Next.js), pour ne pas bloquer la transaction M02/M12.
...
**Multi-vélo (arbitrage Val 3, 2026-05-29) : annuler la collecte annule TOUTES ses missions Everest actives** (les N vélos). Le trigger enqueue **un job `m14_cancel_mission` par mission active** retournée (boucle), pas un seul.
