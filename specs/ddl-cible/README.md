...
- **93 tables** (CREATE TABLE) : **58 `plateforme.*`** (dont 2 `IF NOT EXISTS`), **33 `tms.*`** (dont `tms.audit_logs`, ex-`shared.audit_logs` rapatriée 2026-06-11 ; + `tms.outbox_events` regel 2026-07-06), **2 `shared.*`** (`fichiers`, `prestataires`). *(+1 table 2026-06-10 : `plateforme.jobs_pdf` — challenge Frontière G1, file PDF manquante.)*
- **56 types** ENUM + **162 foreign keys** (section dédiée, ordre-indépendant ; `tms.audit_logs.acteur_user_id` sans FK — snapshot append-only). *(+2 enums 2026-06-10 : `statut_verification_siret`/`statut_verification_tva` + 4 colonnes `entites_facturation` — challenge onboarding ; +1 enum `type_document_pdf` + FK `jobs_pdf.fichier_id → shared.fichiers` — challenge Frontière. +1 enum 2026-06-19 : `mode_facturation_zd_enum` patch M1.7.)*
- Validé par le **vrai parseur PostgreSQL** (libpg_query / pglast v7.15) : grammaire OK (**320 statements**), tous les types définis (56 enums), toutes les FK (162) pointent vers une table+colonne existante, **0 partition** (`PARTITION OF/BY` absents du fichier — les mentions « incl. 2 partitions audit » des changelogs 2026-06-24/2026-07-05 ci-dessous sont périmées), cloisonnement cross-schema respecté (seules les FK vers `shared.*` traversent les schémas). Dernière revalidation : **2026-07-06** (regel `tms.outbox_events` + `collecte_tournees.statut_execution` + `pesees.idempotency_key` sans default — comptes réconciliés COH-11, cet en-tête et celui du `.sql` font foi).
...
**Regelé le 2026-07-05 (patch divergence M3.1 — « Mon profil », BL-P1-TRAIT-02)** : colonne `telephone text` (nullable) ajoutée à `plateforme.users` — champ téléphone éditable du profil utilisateur, transverse tous rôles (§06.04 §7). La table n'avait aucune colonne téléphone (seule `organisations.telephone` existait), provoquant un bug P0 transverse sur `/api/me/profil` (colonne fantôme). Intégrée à la cible (convergence : V1 ⊂ cible rétabli, ce n'est **plus** une divergence à tracer — pas d'ajout à la liste V1-only). V1 migration `20260705110000_plateforme_r19_users_telephone.sql`. **Revalidé pglast v7.14 : 319 statements OK, 92 `CreateStmt` (58 plateforme + 32 tms + 2 shared, incl. 2 partitions audit) *(mention partitions périmée depuis le regel 2026-07-06 — 0 `PARTITION OF/BY` dans le fichier actuel, cf. en-tête)*, 56 enums, 3 schémas — comptes inchangés (une colonne nullable n'ajoute aucun statement).**

**Regelé le 2026-07-06 (revue adversariale concurrence M04/M05 — arbitrages Val RC-M04-06 / RC-M05-01 / RC-M05-02)** : + table **`tms.outbox_events`** (outbox transactionnelle webhooks sortants S1-S11, étendue jobs `everest.*` COH-03 — §04 TMS), + colonne **`tms.collecte_tournees.statut_execution`** (text NOT NULL default `'a_faire'`, CHECK 3 valeurs), **`tms.pesees.idempotency_key` default retiré** (un oubli de propagation doit échouer bruyamment, pas générer une clé fraîche qui tue la dédup). **Revalidé pglast v7.15 : 320 statements OK, 93 CREATE TABLE (58 plateforme + 33 tms + 2 shared), 56 enums, 162 FK, 0 partition, cloisonnement cross-schema OK.** Comptes canoniques réconciliés (COH-11 — l'ex-en-tête « 88 tables / 158 FK / 319 stmts » avait dérivé).
