...
2. **FK contenant → `type_contenant_id` FK `types_contenants`** (noms canoniques §04 conservés ; renommage refusé car `types_contenants` = référentiel de *types* avec tare, plus exact, et `type_contenant_id` est utilisé par M09/M10). Prose M05 (E6, W5, D9, §15.1, Liens) alignée sur §04.
