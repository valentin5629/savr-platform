...
**F3 — `dedup_key`** : DDL complété — `GENERATED ALWAYS AS (code || ':' || COALESCE(entity_type,'') || ':' || COALESCE(entity_id::text,'')) STORED` (aligné v_dedup_key W1). Propagé M11 §11.3 + §04. _(Bonus : résidu `'test'` dans l'enum entity_type §04 corrigé — retiré Bloc 4 A5.)_
