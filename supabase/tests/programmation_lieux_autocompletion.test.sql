...
-- =============================================================================
-- CE QUE CE FICHIER AJOUTE, ET POURQUOI IL NE DOUBLONNE RIEN.
--
-- Trois garde-fous existent autour de cette route, et ils prouvent trois choses
-- DIFFÉRENTES. Aucun ne remplace les deux autres :
--
--   • `lieux_traiteur_operationnel.test.sql` (#371) — prouve la POLICY dans
--     l'abstrait, sur `plateforme.lieux` : l'étendue de la 4e branche.
--   • `lieux_clients_select_cliquet.test.sql` (#374) — pine le TEXTE du prédicat
--     (patterns + empreinte md5) pour forcer la revue du chemin admin support
--     quand la policy change. Il ne tourne sous `authenticated` à aucun assert :
--     par construction, il ne prouve aucun comportement.
--   • CE FICHIER — prouve le COMPORTEMENT de la requête RÉELLE de
--     l'autocomplétion : la vue liste blanche `v_lieux_clients`, la projection
--     exacte de la route, et le filtre `actif = true`, sous l'identité de
--     l'appelant. C'est le seul endroit où l'on vérifie qu'un traiteur
...
-- fichier resterait vert alors que l'écran serait faux.
--
...
--   donnée existe et que la requête n'est pas vide pour d'autres raisons — un
--   négatif sur un ensemble VIDE passerait aussi si le LECTEUR cassait.
-- =============================================================================
